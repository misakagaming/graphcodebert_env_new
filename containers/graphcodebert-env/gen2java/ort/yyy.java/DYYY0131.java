
package gen.ort.yyy;

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
//
//                    Source Code Generated by
//                           CA Gen 8.6
//
//    Copyright (c) 2023 CA Technologies. All rights reserved.
//
//    Name: DYYY0131_PARENT_UPDATE           Date: 2023/07/05
//    Target OS:   JVM                       Time: 15:08:21
//    Target DBMS: JDBC                      User: HakanBal
//    Access Method: JDBC           
//
//    Generation options:
//    Debug trace option selected
//    Data modeling constraint enforcement not selected
//    Optimized import view initialization not selected
//    Enforce default values with DBMS not selected
//    Init unspecified optional fields to NULL not selected
//
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// Import Statements
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
import java.lang.*;
import java.io.*;
import java.util.*;
import java.math.*;
import java.sql.*;
import com.ca.gen.vwrt.*;
import com.ca.gen.vwrt.types.*;
import com.ca.gen.vwrt.vdf.*;

import com.ca.gen.abrt.*;
import com.ca.gen.abrt.functions.*;
import com.ca.gen.abrt.cascade.*;
import com.ca.gen.abrt.trace.*;
import com.ca.gen.exits.common.*;
import com.ca.gen.odc.*;

public class DYYY0131 extends ABBase
{
  // * * * * * * * * * * * * * * * * * * *
  // ENTITY ACTION STATEMENT STATUS FLAGS 
  // ENTITY VIEW STATUS FLAGS AND         
  // LAST COMMAND FOR EACH ENTITY VIEW    
  // * * * * * * * * * * * * * * * * * * *
  String sl_29360149 = ErrorData.NONE_FL_LS;
  String sl_29360136 = ErrorData.NONE_FL_LS;
  String up_27263003 = ErrorData.NONE_FL_LS;
  String Parent_es;
  String Parent_001cd;
  String Parent_lk;
  String Type_es;
  String Type_003cd;
  String Type_lk;
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // IMPORT VIEW CLASS VARIABLE
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  DYYY0131_IA w_ia;
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // EXPORT VIEW CLASS VARIABLE
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  DYYY0131_OA w_oa;
  
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // START OF ENTITY ACTION VIEW
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  /**
   * Internal data view storage for: DYYY0131_EA
   **/
  public class DYYY0131_EA extends ViewBase implements IEntityActionView, Serializable
  {
    // Entity View: 
    //        Type: PARENT
    /**
     * Attribute missing flag for: ParentPinstanceId
     **/
    public char ParentPinstanceId_AS;
    /**
     * Attribute for: ParentPinstanceId
     * Domain: Timestamp
     * Length: 20
     **/
    public String ParentPinstanceId;
    /**
     * Attribute missing flag for: ParentPreferenceId
     **/
    public char ParentPreferenceId_AS;
    /**
     * Attribute for: ParentPreferenceId
     * Domain: Timestamp
     * Length: 20
     **/
    public String ParentPreferenceId;
    /**
     * Attribute missing flag for: ParentPupdateUserId
     **/
    public char ParentPupdateUserId_AS;
    /**
     * Attribute for: ParentPupdateUserId
     * Domain: Text
     * Length: 8
     * Varying Length: N
     **/
    public String ParentPupdateUserId;
    /**
     * Attribute missing flag for: ParentPkeyAttrText
     **/
    public char ParentPkeyAttrText_AS;
    /**
     * Attribute for: ParentPkeyAttrText
     * Domain: Text
     * Length: 5
     * Varying Length: N
     **/
    public String ParentPkeyAttrText;
    /**
     * Attribute missing flag for: ParentPsearchAttrText
     **/
    public char ParentPsearchAttrText_AS;
    /**
     * Attribute for: ParentPsearchAttrText
     * Domain: Text
     * Length: 25
     * Varying Length: N
     **/
    public String ParentPsearchAttrText;
    /**
     * Attribute missing flag for: ParentPotherAttrText
     **/
    public char ParentPotherAttrText_AS;
    /**
     * Attribute for: ParentPotherAttrText
     * Domain: Text
     * Length: 25
     * Varying Length: N
     **/
    public String ParentPotherAttrText;
    /**
     * Attribute missing flag for: ParentPtypeTkeyAttrText
     **/
    public char ParentPtypeTkeyAttrText_AS;
    /**
     * Attribute for: ParentPtypeTkeyAttrText
     * Domain: Text
     * Length: 4
     * Varying Length: N
     **/
    public String ParentPtypeTkeyAttrText;
    // Entity View: 
    //        Type: TYPE
    /**
     * Attribute missing flag for: TypeTkeyAttrText
     **/
    public char TypeTkeyAttrText_AS;
    /**
     * Attribute for: TypeTkeyAttrText
     * Domain: Text
     * Length: 4
     * Varying Length: N
     **/
    public String TypeTkeyAttrText;
    /**
     * Default Constructor
     **/
    
    public DYYY0131_EA()
    {
      reset();
    }
    /**
     * Copy Constructor
     **/
    
    public DYYY0131_EA(DYYY0131_EA orig)
    {
      ParentPinstanceId_AS = orig.ParentPinstanceId_AS;
      ParentPinstanceId = orig.ParentPinstanceId;
      ParentPreferenceId_AS = orig.ParentPreferenceId_AS;
      ParentPreferenceId = orig.ParentPreferenceId;
      ParentPupdateUserId_AS = orig.ParentPupdateUserId_AS;
      ParentPupdateUserId = orig.ParentPupdateUserId;
      ParentPkeyAttrText_AS = orig.ParentPkeyAttrText_AS;
      ParentPkeyAttrText = orig.ParentPkeyAttrText;
      ParentPsearchAttrText_AS = orig.ParentPsearchAttrText_AS;
      ParentPsearchAttrText = orig.ParentPsearchAttrText;
      ParentPotherAttrText_AS = orig.ParentPotherAttrText_AS;
      ParentPotherAttrText = orig.ParentPotherAttrText;
      ParentPtypeTkeyAttrText_AS = orig.ParentPtypeTkeyAttrText_AS;
      ParentPtypeTkeyAttrText = orig.ParentPtypeTkeyAttrText;
      TypeTkeyAttrText_AS = orig.TypeTkeyAttrText_AS;
      TypeTkeyAttrText = orig.TypeTkeyAttrText;
    }
    /**
     * clone constructor
     **/
    
    @Override public Object clone()
    	throws CloneNotSupportedException
    {
      return(new DYYY0131_EA(this));
    }
    /**
     * Resets all properties to the defaults.
     **/
    
    public void reset()
    {
      ParentPinstanceId_AS = ' ';
      ParentPinstanceId = "00000000000000000000";
      ParentPreferenceId_AS = ' ';
      ParentPreferenceId = "00000000000000000000";
      ParentPupdateUserId_AS = ' ';
      ParentPupdateUserId = "        ";
      ParentPkeyAttrText_AS = ' ';
      ParentPkeyAttrText = "     ";
      ParentPsearchAttrText_AS = ' ';
      ParentPsearchAttrText = "                         ";
      ParentPotherAttrText_AS = ' ';
      ParentPotherAttrText = "                         ";
      ParentPtypeTkeyAttrText_AS = ' ';
      ParentPtypeTkeyAttrText = "    ";
      TypeTkeyAttrText_AS = ' ';
      TypeTkeyAttrText = "    ";
    }
  }
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // ENTITY ACTION VIEW CLASS VARIABLE
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  DYYY0131_EA w_ea = new DYYY0131_EA();
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // LOCAL VIEW CLASS VARIABLE
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  DYYY0131_LA w_la;
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // CURSOR OPEN FLAGS
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  boolean curs_open_0029360149 = false;
  boolean curs_open_0029360136 = false;
  
  
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // HOST VARIABLES FOR TABLE: VDVYYYP
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  String pkey_attr_001EF;
  String pkey_attr_002EN;
  String psearch_attr_003EF;
  String psearch_attr_004EN;
  String pother_attr_005EF;
  String pother_attr_006EN;
  String pupdate_userid_007EF;
  String pupdate_userid_008EN;
  java.sql.Timestamp pinstance_id_009EF;
  java.sql.Timestamp pinstance_id_010EN;
  java.sql.Timestamp preference_id_011EF;
  java.sql.Timestamp preference_id_012EN;
  String ptype_tkey_attr_te_013EF;
  String ptype_tkey_attr_te_014EN;
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // HOST VARIABLES FOR TABLE: VDVYYYT
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  String tkey_attr_015EF;
  String tkey_attr_016EN;
  
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // TEMPORARY HOST VARIABLES 
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  String PkeyAttrText_001TP;
  String PtypeTkeyAttrText_002TP;
  
  
  
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // MISC DECLARATIONS AND PROTOTYPES 
  //    FOLLOW AS NEEDED:             
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  boolean func_0022020301_esc_flag;
  PreparedStatement hstmt_0037926947_1;
  ResultSet hstmt_0037926947_1_rs;
  int hstmt_0037926947_1_cnt;
  boolean func_0020971656_esc_flag;
  PreparedStatement hstmt_0037954566_1;
  ResultSet hstmt_0037954566_1_rs;
  int hstmt_0037954566_1_cnt;
  boolean func_0020972221_esc_flag;
  boolean func_0020972469_esc_flag;
  PreparedStatement hstmt_0027263003_1;
  ResultSet hstmt_0027263003_1_rs;
  int hstmt_0027263003_1_cnt;
  PadView padview[] = new PadView [63];
  int trace_ret_code;
  PadText padtext[] = initPadText();
  public PadText [] initPadText()
  {
    PadText [] result = new PadText [PADTEXT_SIZE];
    result[0] = PadText.getInstance(   0,"+->   DYYY0131_PARENT_UPDATE            07/05/2023  15:08");
    result[1] = PadText.getInstance(   0,"!       IMPORTS:");
    result[2] = PadText.getInstance(   0,"!         Work View imp_error iyy1_component (Transient, Mandatory, Import only)");
    result[3] = PadText.getInstance(   0,"!           severity_code");
    result[4] = PadText.getInstance(   0,"!           rollback_indicator");
    result[5] = PadText.getInstance(   0,"!           origin_servid");
    result[6] = PadText.getInstance(   0,"!           context_string");
    result[7] = PadText.getInstance(   0,"!           return_code");
    result[8] = PadText.getInstance(   0,"!           reason_code");
    result[9] = PadText.getInstance(   0,"!           checksum");
    result[10] = PadText.getInstance(   0,"!         Work View imp_reference iyy1_server_data (Transient, Mandatory, Import only)");
    result[11] = PadText.getInstance(   0,"!           userid");
    result[12] = PadText.getInstance(   0,"!           reference_id");
    result[13] = PadText.getInstance(   0,"!         Entity View imp parent (Transient, Mandatory, Import only)");
    result[14] = PadText.getInstance(   0,"!           pinstance_id");
    result[15] = PadText.getInstance(   0,"!           preference_id");
    result[16] = PadText.getInstance(   0,"!           pkey_attr_text");
    result[17] = PadText.getInstance(   0,"!           psearch_attr_text");
    result[18] = PadText.getInstance(   0,"!           pother_attr_text");
    result[19] = PadText.getInstance(   0,"!           ptype_tkey_attr_text");
    result[20] = PadText.getInstance(   0,"!       EXPORTS:");
    result[21] = PadText.getInstance(   0,"!         Entity View exp parent (Transient, Export only)");
    result[22] = PadText.getInstance(   0,"!           preference_id");
    result[23] = PadText.getInstance(   0,"!         Work View exp_error iyy1_component (Transient, Export only)");
    result[24] = PadText.getInstance(   0,"!           severity_code");
    result[25] = PadText.getInstance(   0,"!           rollback_indicator");
    result[26] = PadText.getInstance(   0,"!           origin_servid");
    result[27] = PadText.getInstance(   0,"!           context_string");
    result[28] = PadText.getInstance(   0,"!           return_code");
    result[29] = PadText.getInstance(   0,"!           reason_code");
    result[30] = PadText.getInstance(   0,"!           checksum");
    result[31] = PadText.getInstance(   0,"!       ENTITY ACTIONS:");
    result[32] = PadText.getInstance(   0,"!         Entity View parent");
    result[33] = PadText.getInstance(   0,"!           pinstance_id");
    result[34] = PadText.getInstance(   0,"!           preference_id");
    result[35] = PadText.getInstance(   0,"!           pupdate_user_id");
    result[36] = PadText.getInstance(   0,"!           pkey_attr_text");
    result[37] = PadText.getInstance(   0,"!           psearch_attr_text");
    result[38] = PadText.getInstance(   0,"!           pother_attr_text");
    result[39] = PadText.getInstance(   0,"!           ptype_tkey_attr_text");
    result[40] = PadText.getInstance(   0,"!         Entity View type");
    result[41] = PadText.getInstance(   0,"!           tkey_attr_text");
    result[42] = PadText.getInstance(   0,"!       LOCALS:");
    result[43] = PadText.getInstance(   0,"!         Work View loc dont_change_return_codes");
    result[44] = PadText.getInstance(   0,"!           1_ok");
    result[45] = PadText.getInstance(   0,"!           n10_obj_not_found");
    result[46] = PadText.getInstance(   0,"!           n41_obj_update_failed");
    result[47] = PadText.getInstance(   0,"!         Work View loc dont_change_reason_codes");
    result[48] = PadText.getInstance(   0,"!           1_default");
    result[49] = PadText.getInstance(   0,"!           101_parent_not_found");
    result[50] = PadText.getInstance(   0,"!           102_parent_already_exist");
    result[51] = PadText.getInstance(   0,"!           103_parent_attr_value_invalid");
    result[52] = PadText.getInstance(   0,"!           111_parents_type_not_found");
    result[53] = PadText.getInstance(   0,"!           112_parent_concurrency_error");
    result[54] = PadText.getInstance(   0,"!");
    result[55] = PadText.getInstance(   0,"!     PROCEDURE STATEMENTS");
    result[56] = PadText.getInstance(   0,"!");
    result[57] = PadText.getInstance(   1,"!  NOTE: ");
    result[58] = PadText.getInstance(   1,"!  See the description for the purpose");
    result[59] = PadText.getInstance(   1,"!  ");
    result[60] = PadText.getInstance(   2,"!  NOTE: ");
    result[61] = PadText.getInstance(   2,"!  RELEASE HISTORY");
    result[62] = PadText.getInstance(   2,"!  01_00 23-02-1998 New release");
    result[63] = PadText.getInstance(   2,"!  ");
    result[64] = PadText.getInstance(   3,"!  MOVE imp_error iyy1_component TO exp_error iyy1_component");
    result[65] = PadText.getInstance(   4,"!   ");
    result[66] = PadText.getInstance(   5,"!  NOTE: ");
    result[67] = PadText.getInstance(   5,"!  ****************************************************************");
    result[68] = PadText.getInstance(   5,"!  The used ReturnCode/ReasonCode values");
    result[69] = PadText.getInstance(   5,"!  ");
    result[70] = PadText.getInstance(   6,"!  NOTE: ");
    result[71] = PadText.getInstance(   6,"!  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    result[72] = PadText.getInstance(   6,"!  Set the return and reason codes below");
    result[73] = PadText.getInstance(   6,"!  ");
    result[74] = PadText.getInstance(   7,"!  SET loc dont_change_return_codes 1_ok TO 1 ");
    result[75] = PadText.getInstance(   8,"!  SET loc dont_change_return_codes n10_obj_not_found TO -10 ");
    result[76] = PadText.getInstance(   9,"!  SET loc dont_change_return_codes n41_obj_update_failed TO -41 ");
    result[77] = PadText.getInstance(  10,"!   ");
    result[78] = PadText.getInstance(  11,"!  SET loc dont_change_reason_codes 1_default TO 1 ");
    result[79] = PadText.getInstance(  12,"!  SET loc dont_change_reason_codes 101_parent_not_found TO 101 ");
    result[80] = PadText.getInstance(  13,"!  SET loc dont_change_reason_codes 102_parent_already_exist TO 102 ");
    result[81] = PadText.getInstance(  14,"!  SET loc dont_change_reason_codes 103_parent_attr_value_invalid TO 103 ");
    result[82] = PadText.getInstance(  15,"!  SET loc dont_change_reason_codes 111_parents_type_not_found TO 111 ");
    result[83] = PadText.getInstance(  16,"!  SET loc dont_change_reason_codes 112_parent_concurrency_error TO 112 ");
    result[84] = PadText.getInstance(  17,"!   ");
    result[85] = PadText.getInstance(  18,"!  NOTE: ");
    result[86] = PadText.getInstance(  18,"!  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    result[87] = PadText.getInstance(  18,
      "!  If the Instance_id is being used, reading must be operated according to this section, instead of business_key(s).");
    result[88] = PadText.getInstance(  18,"!  ");
    result[89] = PadText.getInstance(  19,"!  NOTE: ");
    result[90] = PadText.getInstance(  19,"!  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    result[91] = PadText.getInstance(  19,
      "!  Choose Control of Cursor Generation property as \"Select only\". (being read for update)");
    result[92] = PadText.getInstance(  19,"!  ");
    result[93] = PadText.getInstance(  20,"!  +=>READ (Cursor Only) parent");
    result[94] = PadText.getInstance(  20,"!  !        WHERE DESIRED parent pkey_attr_text = imp parent pkey_attr_text");
    result[95] = PadText.getInstance(  20, 1, "!  +> WHEN successful");
    result[96] = PadText.getInstance(  21,"!  !  +->IF parent preference_id ^= imp parent preference_id");
    result[97] = PadText.getInstance(  22,
      "!  !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n41_obj_update_failed ");
    result[98] = PadText.getInstance(  23,
      "!  !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 112_parent_concurrency_error ");
    result[99] = PadText.getInstance(  24,"! <------ESCAPE");
    result[100] = PadText.getInstance(  21,"!  !  +--");
    result[101] = PadText.getInstance(  25,"!  !  NOTE: ");
    result[102] = PadText.getInstance(  25,"!  !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    result[103] = PadText.getInstance(  25,
      "!  !  If the operation 'type' depends on the data type, protect the 'READ type' method below; remove if not.");
    result[104] = PadText.getInstance(  25,"!  !  ");
    result[105] = PadText.getInstance(  26,"!  !  NOTE: ");
    result[106] = PadText.getInstance(  26,
      "!  !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    result[107] = PadText.getInstance(  26,"!  !  Choose Control of Cursor Generation property as \"Select only\".");
    result[108] = PadText.getInstance(  26,"!  !  ");
    result[109] = PadText.getInstance(  27,"!  !  +=>READ (Select Only) type");
    result[110] = PadText.getInstance(  27,"!  !  !        WHERE DESIRED type tkey_attr_text = imp parent ptype_tkey_attr_text");
    result[111] = PadText.getInstance(  27, 1, "!  !  +> WHEN successful");
    result[112] = PadText.getInstance(  28,"!  !  !  NOTE: ");
    result[113] = PadText.getInstance(  28,
      "!  !  !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    result[114] = PadText.getInstance(  28,
      "!  !  !  All fields are updated except for the key field(s), Create_ts and Create_userid. ");
    result[115] = PadText.getInstance(  28,"!  !  !  ");
    result[116] = PadText.getInstance(  29,"!  !  !  +->UPDATE parent");
    result[117] = PadText.getInstance(  30,"!  !  !  !  SET preference_id TO imp_reference iyy1_server_data reference_id ");
    result[118] = PadText.getInstance(  31,"!  !  !  !  SET pupdate_user_id TO imp_reference iyy1_server_data userid ");
    result[119] = PadText.getInstance(  32,"!  !  !  !  SET psearch_attr_text TO imp parent psearch_attr_text ");
    result[120] = PadText.getInstance(  33,"!  !  !  !  SET pother_attr_text TO imp parent pother_attr_text ");
    result[121] = PadText.getInstance(  34,"!  !  !  !  SET ptype_tkey_attr_text TO imp parent ptype_tkey_attr_text ");
    result[122] = PadText.getInstance(  29, 1, "!  !  !  +> WHEN successful");
    result[123] = PadText.getInstance(  35,"!  !  !  !  MOVE  parent TO exp parent");
    result[124] = PadText.getInstance(  29, 2, "!  !  !  +> WHEN not unique");
    result[125] = PadText.getInstance(  36,
      "!  !  !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n41_obj_update_failed ");
    result[126] = PadText.getInstance(  37,
      "!  !  !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 102_parent_already_exist ");
    result[127] = PadText.getInstance(  29, 3, "!  !  !  +> WHEN permitted value violation");
    result[128] = PadText.getInstance(  38,
      "!  !  !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n41_obj_update_failed ");
    result[129] = PadText.getInstance(  39,
      "!  !  !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 103_parent_attr_value_invalid ");
    result[130] = PadText.getInstance(  29,"!  !  !  +--");
    result[131] = PadText.getInstance(  27, 2, "!  !  +> WHEN not found");
    result[132] = PadText.getInstance(  40,
      "!  !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n41_obj_update_failed ");
    result[133] = PadText.getInstance(  41,
      "!  !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 111_parents_type_not_found ");
    result[134] = PadText.getInstance(  27,"!  !  +--");
    result[135] = PadText.getInstance(  20, 2, "!  +> WHEN not found");
    result[136] = PadText.getInstance(  42,
      "!  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n10_obj_not_found ");
    result[137] = PadText.getInstance(  43,
      "!  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 101_parent_not_found ");
    result[138] = PadText.getInstance(  20,"!  +--");
    result[139] = PadText.getInstance(   0,"+---");
    result[140] = PadText.getInstance(PadText.LASTLINE,"************************* BOTTOM OF DATA *************************");
    return result;
  }
  final static int PADTEXT_SIZE = 141;
  //       +->   DYYY0131_PARENT_UPDATE            07/05/2023  15:08
  //       !       IMPORTS:
  //       !         Work View imp_error iyy1_component (Transient, Mandatory, Import only)
  //       !           severity_code
  //       !           rollback_indicator
  //       !           origin_servid
  //       !           context_string
  //       !           return_code
  //       !           reason_code
  //       !           checksum
  //       !         Work View imp_reference iyy1_server_data (Transient, Mandatory, Import only)
  //       !           userid
  //       !           reference_id
  //       !         Entity View imp parent (Transient, Mandatory, Import only)
  //       !           pinstance_id
  //       !           preference_id
  //       !           pkey_attr_text
  //       !           psearch_attr_text
  //       !           pother_attr_text
  //       !           ptype_tkey_attr_text
  //       !       EXPORTS:
  //       !         Entity View exp parent (Transient, Export only)
  //       !           preference_id
  //       !         Work View exp_error iyy1_component (Transient, Export only)
  //       !           severity_code
  //       !           rollback_indicator
  //       !           origin_servid
  //       !           context_string
  //       !           return_code
  //       !           reason_code
  //       !           checksum
  //       !       ENTITY ACTIONS:
  //       !         Entity View parent
  //       !           pinstance_id
  //       !           preference_id
  //       !           pupdate_user_id
  //       !           pkey_attr_text
  //       !           psearch_attr_text
  //       !           pother_attr_text
  //       !           ptype_tkey_attr_text
  //       !         Entity View type
  //       !           tkey_attr_text
  //       !       LOCALS:
  //       !         Work View loc dont_change_return_codes
  //       !           1_ok
  //       !           n10_obj_not_found
  //       !           n41_obj_update_failed
  //       !         Work View loc dont_change_reason_codes
  //       !           1_default
  //       !           101_parent_not_found
  //       !           102_parent_already_exist
  //       !           103_parent_attr_value_invalid
  //       !           111_parents_type_not_found
  //       !           112_parent_concurrency_error
  //       !
  //       !     PROCEDURE STATEMENTS
  //       !
  //     1 !  NOTE: 
  //     1 !  See the description for the purpose
  //     1 !  
  //     2 !  NOTE: 
  //     2 !  RELEASE HISTORY
  //     2 !  01_00 23-02-1998 New release
  //     2 !  
  //     3 !  MOVE imp_error iyy1_component TO exp_error iyy1_component
  //     4 !   
  //     5 !  NOTE: 
  //     5 !  ****************************************************************
  //     5 !  The used ReturnCode/ReasonCode values
  //     5 !  
  //     6 !  NOTE: 
  //     6 !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //     6 !  Set the return and reason codes below
  //     6 !  
  //     7 !  SET loc dont_change_return_codes 1_ok TO 1 
  //     8 !  SET loc dont_change_return_codes n10_obj_not_found TO -10 
  //     9 !  SET loc dont_change_return_codes n41_obj_update_failed TO -41 
  //    10 !   
  //    11 !  SET loc dont_change_reason_codes 1_default TO 1 
  //    12 !  SET loc dont_change_reason_codes 101_parent_not_found TO 101 
  //    13 !  SET loc dont_change_reason_codes 102_parent_already_exist TO 102 
  //    14 !  SET loc dont_change_reason_codes 103_parent_attr_value_invalid TO 103 
  //    15 !  SET loc dont_change_reason_codes 111_parents_type_not_found TO 111 
  //    16 !  SET loc dont_change_reason_codes 112_parent_concurrency_error TO 112 
  //    17 !   
  //    18 !  NOTE: 
  //    18 !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //    18 !  If the Instance_id is being used, reading must be operated according to this section, instead of business_key(s).
  //    18 !  
  //    19 !  NOTE: 
  //    19 !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //    19 !  Choose Control of Cursor Generation property as "Select only". (being read for update)
  //    19 !  
  //    20 !  +=>READ (Cursor Only) parent
  //    20 !  !        WHERE DESIRED parent pkey_attr_text = imp parent pkey_attr_text
  //    20 !  +> WHEN successful
  //    21 !  !  +->IF parent preference_id ^= imp parent preference_id
  //    22 !  !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n41_obj_update_failed 
  //    23 !  !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 112_parent_concurrency_error 
  //    24 ! <------ESCAPE
  //    21 !  !  +--
  //    25 !  !  NOTE: 
  //    25 !  !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //    25 !  !  If the operation 'type' depends on the data type, protect the 'READ type' method below; remove if not.
  //    25 !  !  
  //    26 !  !  NOTE: 
  //    26 !  !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //    26 !  !  Choose Control of Cursor Generation property as "Select only".
  //    26 !  !  
  //    27 !  !  +=>READ (Select Only) type
  //    27 !  !  !        WHERE DESIRED type tkey_attr_text = imp parent ptype_tkey_attr_text
  //    27 !  !  +> WHEN successful
  //    28 !  !  !  NOTE: 
  //    28 !  !  !  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //    28 !  !  !  All fields are updated except for the key field(s), Create_ts and Create_userid. 
  //    28 !  !  !  
  //    29 !  !  !  +->UPDATE parent
  //    30 !  !  !  !  SET preference_id TO imp_reference iyy1_server_data reference_id 
  //    31 !  !  !  !  SET pupdate_user_id TO imp_reference iyy1_server_data userid 
  //    32 !  !  !  !  SET psearch_attr_text TO imp parent psearch_attr_text 
  //    33 !  !  !  !  SET pother_attr_text TO imp parent pother_attr_text 
  //    34 !  !  !  !  SET ptype_tkey_attr_text TO imp parent ptype_tkey_attr_text 
  //    29 !  !  !  +> WHEN successful
  //    35 !  !  !  !  MOVE  parent TO exp parent
  //    29 !  !  !  +> WHEN not unique
  //    36 !  !  !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n41_obj_update_failed 
  //    37 !  !  !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 102_parent_already_exist 
  //    29 !  !  !  +> WHEN permitted value violation
  //    38 !  !  !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n41_obj_update_failed 
  //    39 !  !  !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 103_parent_attr_value_invalid 
  //    29 !  !  !  +--
  //    27 !  !  +> WHEN not found
  //    40 !  !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n41_obj_update_failed 
  //    41 !  !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 111_parents_type_not_found 
  //    27 !  !  +--
  //    20 !  +> WHEN not found
  //    42 !  !  SET exp_error iyy1_component return_code TO loc dont_change_return_codes n10_obj_not_found 
  //    43 !  !  SET exp_error iyy1_component reason_code TO loc dont_change_reason_codes 101_parent_not_found 
  //    20 !  +--
  //       +---
  
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  //  CONSTRUCTOR FOR THE CLASS       
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  public DYYY0131()
  {
    iefCGenRlse = "CA Gen 8.6";
    ssCopyright = "Copyright (c) 2023 CA Technologies. All rights reserved.";
    iefCGenDate = "2023/07/05";
    iefCGenTime = "15:08:21";
    iefCGenEncy = "9.2.A6";
    iefCGenUserId = "HakanBal";
    iefCGenModel = "N8I_ORT_YYY_0112_TEMPLATE";
    iefCGenSubset = "ALL";
    iefCGenName = "DYYY0131_PARENT_UPDATE";
    nestingLevel = 0;
    valChkDeadlockTimeout = false;
    valChkDBError = false;
  }
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // ACTION BLOCK FUNCTION DECLARATIONS 
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  
  public void DYYY0131( Object in_runtime_parm1, 
  	IRuntimePStepContext in_runtime_parm2, 
  	GlobData in_globdata, 
  	DYYY0131_IA import_view, 
  	DYYY0131_OA export_view )
  {
    iefRuntimeParm1 = in_runtime_parm1;
    iefRuntimeParm2 = in_runtime_parm2;
    globdata = in_globdata;
    w_ia = import_view;
    w_oa = export_view;
    _DYYY0131();
  }
  
  private void _DYYY0131()
  {
    
    f_22020301_localAlloc("DYYY0131_PARENT_UPDATE");
    if ( globdata.getErrorData().getLastStatus() == ErrorData.IEF_ALLOCATION_ERROR_FL_LS )
    	return;
    
    ++(nestingLevel);
    try {
      f_22020301_init();
      f_22020301();
    } catch( ABException e ) {
    } finally {
      --(nestingLevel);
    }
    String last_stmt_num_saved = globdata.getStateData().getLastStatementNumber();
    globdata.getStateData().setLastStatementNumber( "9999999999" );
    try {
      f_0_trace();
    } catch( ABException e ) {
    }
    globdata.getStateData().setLastStatementNumber( last_stmt_num_saved );
    
    
    f_22020301_term();
  }
  public void f_22020301()
  	throws ABException
  {
    func_0022020301_esc_flag = false;
    globdata.getStateData().setCurrentABId( "0022020301" );
    globdata.getStateData().setCurrentABName( "DYYY0131_PARENT_UPDATE" );
    padview_init();
    globdata.getStateData().setLastStatementNumber( "0000000000" );
    f_0_trace();
    f_0022020301_esctag: 
    {
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      //    See the description for the purpose                             
      //                                                                    
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      //    RELEASE HISTORY                                                 
      //    01_00 23-02-1998 New release                                    
      //                                                                    
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      globdata.getStateData().setLastStatementNumber( "0000000003" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_oa.ExpErrorIyy1ComponentSeverityCode = FixedStringAttr.valueOf(w_ia.ImpErrorIyy1ComponentSeverityCode, (short)1);
        w_oa.ExpErrorIyy1ComponentRollbackIndicator = FixedStringAttr.valueOf(w_ia.ImpErrorIyy1ComponentRollbackIndicator, (short)1)
          ;
        w_oa.ExpErrorIyy1ComponentOriginServid = DoubleAttr.valueOf(w_ia.ImpErrorIyy1ComponentOriginServid);
        w_oa.ExpErrorIyy1ComponentContextString = StringAttr.valueOf(w_ia.ImpErrorIyy1ComponentContextString, (short)512);
        w_oa.ExpErrorIyy1ComponentReturnCode = IntAttr.valueOf(w_ia.ImpErrorIyy1ComponentReturnCode);
        w_oa.ExpErrorIyy1ComponentReasonCode = IntAttr.valueOf(w_ia.ImpErrorIyy1ComponentReasonCode);
        w_oa.ExpErrorIyy1ComponentChecksum = FixedStringAttr.valueOf(w_ia.ImpErrorIyy1ComponentChecksum, (short)15);
      }
      
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      //    **************************************************************  
      //    **                                                              
      //    The used ReturnCode/ReasonCode values                           
      //                                                                    
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      //    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
      //    !!                                                              
      //    Set the return and reason codes below                           
      //                                                                    
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      globdata.getStateData().setLastStatementNumber( "0000000007" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReturnCodesQ1Ok = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(1, 0, TIRD2DEC.ROUND_NONE, 5));
      }
      globdata.getStateData().setLastStatementNumber( "0000000008" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReturnCodesN10ObjNotFound = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(-10, 0, TIRD2DEC.ROUND_NONE, 5));
      }
      globdata.getStateData().setLastStatementNumber( "0000000009" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReturnCodesN41ObjUpdateFailed = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(-41, 0, TIRD2DEC.ROUND_NONE, 5));
      }
      
      globdata.getStateData().setLastStatementNumber( "0000000011" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReasonCodesQ1Default = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(1, 0, TIRD2DEC.ROUND_NONE, 5));
      }
      globdata.getStateData().setLastStatementNumber( "0000000012" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReasonCodesQ101ParentNotFound = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(101, 0, TIRD2DEC.ROUND_NONE, 5));
      }
      globdata.getStateData().setLastStatementNumber( "0000000013" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReasonCodesQ102ParentAlreadyExist = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(102, 0, TIRD2DEC.ROUND_NONE, 5))
          ;
      }
      globdata.getStateData().setLastStatementNumber( "0000000014" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReasonCodesQ103ParentAttrValueInvalid = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(103, 0, TIRD2DEC.ROUND_NONE,
           5));
      }
      globdata.getStateData().setLastStatementNumber( "0000000015" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReasonCodesQ111ParentsTypeNotFound = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(111, 0, TIRD2DEC.ROUND_NONE, 5)
          );
      }
      globdata.getStateData().setLastStatementNumber( "0000000016" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_la.LocDontChangeReasonCodesQ112ParentConcurrencyError = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1(112, 0, TIRD2DEC.ROUND_NONE,
           5));
      }
      
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      //    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
      //    !!!!!!!!!!!!!!!!!!!!!                                           
      //    If the Instance_id is being used,                               
      //    reading must be operated according to this section,             
      //    instead of business_key(s).                                     
      //                                                                    
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      //    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
      //    !!!!!!!!!!!!!!!!!!!!!                                           
      //    Choose Control of Cursor Generation property as                 
      //    "Select only". (being read for update)                          
      //                                                                    
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      globdata.getStateData().setLastStatementNumber( "0000000020" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        f_29360149();
        if ( sl_29360149.equals( ErrorData.SUCCEEDS_FL_LS ) == true )
        {
          globdata.getStateData().setLastSubStatementNumber( "1" );
          f_0_trace();
          {
            f_20971656();
          }
        }
        else if ( sl_29360149.equals( ErrorData.NOT_FOUND_FL_LS ) == true )
        {
          globdata.getStateData().setLastSubStatementNumber( "2" );
          f_0_trace();
          {
            globdata.getStateData().setLastStatementNumber( "0000000042" );
            f_0_trace();
            if ( trace_ret_code != 8 )
            {
              w_oa.ExpErrorIyy1ComponentReturnCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
                w_la.LocDontChangeReturnCodesN10ObjNotFound, 0, TIRD2DEC.ROUND_NONE, 5));
            }
            globdata.getStateData().setLastStatementNumber( "0000000043" );
            f_0_trace();
            if ( trace_ret_code != 8 )
            {
              w_oa.ExpErrorIyy1ComponentReasonCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
                w_la.LocDontChangeReasonCodesQ101ParentNotFound, 0, TIRD2DEC.ROUND_NONE, 5));
            }
          }
        }
        else {
          globdata.getErrorData().setStatus( ErrorData.FATAL_ERROR_FL_LS );
          globdata.getErrorData().setLastStatus( sl_29360149 );
          globdata.getErrorData().setErrorEncounteredSW( ErrorData.ERROR_FOUND_SW );
          throw new ABException();
        }
      }
    }
  }
  
  
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // SUBORDINATE FUNCTIONS
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  
  
  public void f_20971656()
  	throws ABException
  {
    func_0020971656_esc_flag = false;
    f_0020971656_esctag: 
    {
      globdata.getStateData().setLastStatementNumber( "0000000021" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        if ( CompareExit.compareTo(w_ea.ParentPreferenceId, w_ia.ImpParentPreferenceId) != 0 )
        {
          f_20972221();
        }
        
      }
      if ( func_0020971656_esc_flag )
      {
        break f_0020971656_esctag;
      }
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      //    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
      //    !!!!!!!!!!!!!!                                                  
      //    If the operation 'type' depends on the data type, protect the   
      //    'READ type' method below; remove if not.                        
      //                                                                    
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      //    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
      //    !!!!!!!!!!!!!!!!!!!!!                                           
      //    Choose Control of Cursor Generation property as                 
      //    "Select only".                                                  
      //                                                                    
      // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
      globdata.getStateData().setLastStatementNumber( "0000000027" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        f_29360136();
        if ( sl_29360136.equals( ErrorData.SUCCEEDS_FL_LS ) == true )
        {
          globdata.getStateData().setLastSubStatementNumber( "1" );
          f_0_trace();
          {
            f_20972469();
          }
        }
        else if ( sl_29360136.equals( ErrorData.NOT_FOUND_FL_LS ) == true )
        {
          globdata.getStateData().setLastSubStatementNumber( "2" );
          f_0_trace();
          {
            globdata.getStateData().setLastStatementNumber( "0000000040" );
            f_0_trace();
            if ( trace_ret_code != 8 )
            {
              w_oa.ExpErrorIyy1ComponentReturnCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
                w_la.LocDontChangeReturnCodesN41ObjUpdateFailed, 0, TIRD2DEC.ROUND_NONE, 5));
            }
            globdata.getStateData().setLastStatementNumber( "0000000041" );
            f_0_trace();
            if ( trace_ret_code != 8 )
            {
              w_oa.ExpErrorIyy1ComponentReasonCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
                w_la.LocDontChangeReasonCodesQ111ParentsTypeNotFound, 0, TIRD2DEC.ROUND_NONE, 5));
            }
          }
        }
        else {
          globdata.getErrorData().setStatus( ErrorData.FATAL_ERROR_FL_LS );
          globdata.getErrorData().setLastStatus( sl_29360136 );
          globdata.getErrorData().setErrorEncounteredSW( ErrorData.ERROR_FOUND_SW );
          throw new ABException();
        }
      }
    }
  }
  
  public void f_20972221()
  	throws ABException
  {
    func_0020972221_esc_flag = false;
    f_0020972221_esctag: 
    {
      globdata.getStateData().setLastStatementNumber( "0000000022" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_oa.ExpErrorIyy1ComponentReturnCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
          w_la.LocDontChangeReturnCodesN41ObjUpdateFailed, 0, TIRD2DEC.ROUND_NONE, 5));
      }
      globdata.getStateData().setLastStatementNumber( "0000000023" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        w_oa.ExpErrorIyy1ComponentReasonCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
          w_la.LocDontChangeReasonCodesQ112ParentConcurrencyError, 0, TIRD2DEC.ROUND_NONE, 5));
      }
      globdata.getStateData().setLastStatementNumber( "0000000024" );
      f_0_trace();
      if ( trace_ret_code != 8 )
      {
        func_0020972221_esc_flag = true;
        func_0020971656_esc_flag = true;
      }
    }
  }
  
  public void f_20972469()
  	throws ABException
  {
    func_0020972469_esc_flag = false;
    // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
    //    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
    //    !!!!!!!!!!!!!!!!!!!!!                                           
    //    All fields are updated except for the key field(s),             
    //    Create_ts and Create_userid.                                    
    //                                                                    
    // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
    globdata.getStateData().setLastStatementNumber( "0000000029" );
    f_0_trace();
    if ( trace_ret_code != 8 )
    {
      w_ea.ParentPreferenceId = TimestampAttr.valueOf(w_ia.ImpReferenceIyy1ServerDataReferenceId);
      w_ea.ParentPupdateUserId = FixedStringAttr.valueOf(w_ia.ImpReferenceIyy1ServerDataUserid, (short)8);
      w_ea.ParentPsearchAttrText = FixedStringAttr.valueOf(w_ia.ImpParentPsearchAttrText, (short)25);
      w_ea.ParentPotherAttrText = FixedStringAttr.valueOf(w_ia.ImpParentPotherAttrText, (short)25);
      w_ea.ParentPtypeTkeyAttrText = FixedStringAttr.valueOf(w_ia.ImpParentPtypeTkeyAttrText, (short)4);
      valChkDeadlockTimeout = false;
      valChkDBError = false;
      f_27263003();
      if ( up_27263003.equals( ErrorData.SUCCEEDS_FL_LS ) == true )
      {
        globdata.getStateData().setLastSubStatementNumber( "1" );
        f_0_trace();
        {
          globdata.getStateData().setLastStatementNumber( "0000000035" );
          f_0_trace();
          if ( trace_ret_code != 8 )
          {
            w_oa.ExpParentPreferenceId = TimestampAttr.valueOf(w_ea.ParentPreferenceId);
          }
        }
      }
      else if ( up_27263003.equals( ErrorData.DUPLICATE_FOUND_FL_LS ) == true )
      {
        globdata.getStateData().setLastSubStatementNumber( "2" );
        f_0_trace();
        {
          globdata.getStateData().setLastStatementNumber( "0000000036" );
          f_0_trace();
          if ( trace_ret_code != 8 )
          {
            w_oa.ExpErrorIyy1ComponentReturnCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
              w_la.LocDontChangeReturnCodesN41ObjUpdateFailed, 0, TIRD2DEC.ROUND_NONE, 5));
          }
          globdata.getStateData().setLastStatementNumber( "0000000037" );
          f_0_trace();
          if ( trace_ret_code != 8 )
          {
            w_oa.ExpErrorIyy1ComponentReasonCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
              w_la.LocDontChangeReasonCodesQ102ParentAlreadyExist, 0, TIRD2DEC.ROUND_NONE, 5));
          }
        }
      }
      else if ( up_27263003.equals( ErrorData.INVALID_DATAB_PERM_FL_LS ) == true )
      {
        globdata.getStateData().setLastSubStatementNumber( "3" );
        f_0_trace();
        {
          globdata.getStateData().setLastStatementNumber( "0000000038" );
          f_0_trace();
          if ( trace_ret_code != 8 )
          {
            w_oa.ExpErrorIyy1ComponentReturnCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
              w_la.LocDontChangeReturnCodesN41ObjUpdateFailed, 0, TIRD2DEC.ROUND_NONE, 5));
          }
          globdata.getStateData().setLastStatementNumber( "0000000039" );
          f_0_trace();
          if ( trace_ret_code != 8 )
          {
            w_oa.ExpErrorIyy1ComponentReasonCode = IntAttr.valueOf((int)TIRD2DEC.TIRD2DC1((double) 
              w_la.LocDontChangeReasonCodesQ103ParentAttrValueInvalid, 0, TIRD2DEC.ROUND_NONE, 5));
          }
        }
      }
      else {
        globdata.getErrorData().setStatus( ErrorData.DB_ERROR_FL_LS );
        globdata.getErrorData().setErrorEncounteredSW( ErrorData.ERROR_FOUND_SW );
        throw new ABException();
      }
    }
  }
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  // INITIALIZATION UTILITY FUNCTIONS 
  // * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
  
  public void f_22020301_localAlloc(String abname)
  {
    // Request localview allocation 
    w_la = (gen.ort.yyy.DYYY0131_LA)(iefRuntimeParm2.getInstance("gen.ort.yyy.DYYY0131_LA"));
    if ( w_la == null )
    {
      globdata.getStateData().setCurrentABId( "0022020301" );
      globdata.getStateData().setCurrentABName( abname );
      globdata.getErrorData().setErrorEncounteredSW( ErrorData.ERROR_FOUND_SW );
      globdata.getErrorData().setLastStatus( ErrorData.IEF_ALLOCATION_ERROR_FL_LS );
    }
  }
  
  public void f_22020301_init()
  	throws ABException
  {
    
    pkey_attr_001EF = SPACES;
    pkey_attr_002EN = SPACES;
    psearch_attr_003EF = SPACES;
    psearch_attr_004EN = SPACES;
    pother_attr_005EF = SPACES;
    pother_attr_006EN = SPACES;
    pupdate_userid_007EF = SPACES;
    pupdate_userid_008EN = SPACES;
    pinstance_id_009EF = TIRVW2S.TIRVW2S(globdata, "IEFDB", "00010101000000000000");
    pinstance_id_010EN = TIRVW2S.TIRVW2S(globdata, "IEFDB", "00010101000000000000");
    preference_id_011EF = TIRVW2S.TIRVW2S(globdata, "IEFDB", "00010101000000000000");
    preference_id_012EN = TIRVW2S.TIRVW2S(globdata, "IEFDB", "00010101000000000000");
    ptype_tkey_attr_te_013EF = SPACES;
    ptype_tkey_attr_te_014EN = SPACES;
    
    tkey_attr_015EF = SPACES;
    tkey_attr_016EN = SPACES;
    if ( nestingLevel < 2 )
    {
      w_la.reset();
    }
    w_ea.reset();
    w_oa.ExpParentPreferenceId = "00000000000000000000";
    w_oa.ExpErrorIyy1ComponentSeverityCode = " ";
    w_oa.ExpErrorIyy1ComponentRollbackIndicator = " ";
    w_oa.ExpErrorIyy1ComponentOriginServid = 0.0;
    w_oa.ExpErrorIyy1ComponentContextString = "";
    w_oa.ExpErrorIyy1ComponentReturnCode = 0;
    w_oa.ExpErrorIyy1ComponentReasonCode = 0;
    w_oa.ExpErrorIyy1ComponentChecksum = "               ";
    Parent_es = ABBase.EV_UNUSABLE;
    Parent_lk = ABBase.EVW_UNLOCKED;
    w_ea.ParentPinstanceId = "00000000000000000000";
    w_ea.ParentPreferenceId = "00000000000000000000";
    Type_es = ABBase.EV_UNUSABLE;
    Type_lk = ABBase.EVW_UNLOCKED;
  }
  
  public void f_0_trace()
  	throws ABException
  {
    trace_ret_code = TIRTRCE.TIRTRCE(iefRuntimeParm1, iefRuntimeParm2, globdata, padtext, padview);
    if ( trace_ret_code == 20 )
    {
      throw new ABException();
    }
  }
  
  public void padview_init()
  {
padview[0] = PadView.getInstance("IMPORT                          ", 0, 0, 0, (short)1, PadView.VT_BASE, PadView.DT_NONE, w_ia, 
      "w_ia");
    padview[1] = PadView.getInstance("IMP_ERROR                       ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_ia, 
      null);
    padview[2] = PadView.getInstance("IYY1_COMPONENT                  ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, w_ia,
       null);
    padview[3] = PadView.getInstance("SEVERITY_CODE                   ", 1, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ia, "ImpErrorIyy1ComponentSeverityCode");
    padview[4] = PadView.getInstance("ROLLBACK_INDICATOR              ", 1, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ia, "ImpErrorIyy1ComponentRollbackIndicator");
    padview[5] = PadView.getInstance("ORIGIN_SERVID                   ", 15, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_ia, "ImpErrorIyy1ComponentOriginServid");
    padview[6] = PadView.getInstance("CONTEXT_STRING                  ", 512, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_VARCHAR, w_ia, "ImpErrorIyy1ComponentContextString");
    padview[7] = PadView.getInstance("RETURN_CODE                     ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_NUMERIC,
       w_ia, "ImpErrorIyy1ComponentReturnCode");
    padview[8] = PadView.getInstance("REASON_CODE                     ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_NUMERIC,
       w_ia, "ImpErrorIyy1ComponentReasonCode");
    padview[9] = PadView.getInstance("CHECKSUM                        ", 15, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ia, "ImpErrorIyy1ComponentChecksum");
    padview[10] = PadView.getInstance("IMP_REFERENCE                   ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_ia, 
      null);
    padview[11] = PadView.getInstance("IYY1_SERVER_DATA                ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, 
      w_ia, null);
    padview[12] = PadView.getInstance("USERID                          ", 8, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ia, "ImpReferenceIyy1ServerDataUserid");
    padview[13] = PadView.getInstance("REFERENCE_ID                    ", 20, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_TIMESTAMP, w_ia, "ImpReferenceIyy1ServerDataReferenceId");
    padview[14] = PadView.getInstance("IMP                             ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_ia, 
      null);
    padview[15] = PadView.getInstance("PARENT                          ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, 
      w_ia, null);
    padview[16] = PadView.getInstance("PINSTANCE_ID                    ", 20, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_TIMESTAMP, w_ia, "ImpParentPinstanceId");
    padview[17] = PadView.getInstance("PREFERENCE_ID                   ", 20, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_TIMESTAMP, w_ia, "ImpParentPreferenceId");
    padview[18] = PadView.getInstance("PKEY_ATTR_TEXT                  ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ia, "ImpParentPkeyAttrText");
    padview[19] = PadView.getInstance("PSEARCH_ATTR_TEXT               ", 25, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ia, "ImpParentPsearchAttrText");
    padview[20] = PadView.getInstance("POTHER_ATTR_TEXT                ", 25, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ia, "ImpParentPotherAttrText");
    padview[21] = PadView.getInstance("PTYPE_TKEY_ATTR_TEXT            ", 4, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ia, "ImpParentPtypeTkeyAttrText");
    padview[22] = PadView.getInstance("EXPORT                          ", 0, 0, 0, (short)1, PadView.VT_BASE, PadView.DT_NONE, w_oa, 
      "w_oa");
    padview[23] = PadView.getInstance("EXP                             ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_oa, 
      null);
    padview[24] = PadView.getInstance("PARENT                          ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, 
      w_oa, null);
    padview[25] = PadView.getInstance("PREFERENCE_ID                   ", 20, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_TIMESTAMP, w_oa, "ExpParentPreferenceId");
    padview[26] = PadView.getInstance("EXP_ERROR                       ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_oa, 
      null);
    padview[27] = PadView.getInstance("IYY1_COMPONENT                  ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, 
      w_oa, null);
    padview[28] = PadView.getInstance("SEVERITY_CODE                   ", 1, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_oa, "ExpErrorIyy1ComponentSeverityCode");
    padview[29] = PadView.getInstance("ROLLBACK_INDICATOR              ", 1, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_oa, "ExpErrorIyy1ComponentRollbackIndicator");
    padview[30] = PadView.getInstance("ORIGIN_SERVID                   ", 15, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_oa, "ExpErrorIyy1ComponentOriginServid");
    padview[31] = PadView.getInstance("CONTEXT_STRING                  ", 512, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_VARCHAR, w_oa, "ExpErrorIyy1ComponentContextString");
    padview[32] = PadView.getInstance("RETURN_CODE                     ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_oa, "ExpErrorIyy1ComponentReturnCode");
    padview[33] = PadView.getInstance("REASON_CODE                     ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_oa, "ExpErrorIyy1ComponentReasonCode");
    padview[34] = PadView.getInstance("CHECKSUM                        ", 15, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_oa, "ExpErrorIyy1ComponentChecksum");
    padview[35] = PadView.getInstance("ENTITY                          ", 0, 0, 0, (short)1, PadView.VT_BASE, PadView.DT_NONE, w_ea, 
      "w_ea");
    padview[36] = PadView.getInstance("                                ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_ea, 
      null);
    padview[37] = PadView.getInstance("PARENT                          ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, 
      w_ea, null);
    padview[38] = PadView.getInstance("PINSTANCE_ID                    ", 20, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_TIMESTAMP, w_ea, "ParentPinstanceId");
    padview[39] = PadView.getInstance("PREFERENCE_ID                   ", 20, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_TIMESTAMP, w_ea, "ParentPreferenceId");
    padview[40] = PadView.getInstance("PUPDATE_USER_ID                 ", 8, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ea, "ParentPupdateUserId");
    padview[41] = PadView.getInstance("PKEY_ATTR_TEXT                  ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ea, "ParentPkeyAttrText");
    padview[42] = PadView.getInstance("PSEARCH_ATTR_TEXT               ", 25, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ea, "ParentPsearchAttrText");
    padview[43] = PadView.getInstance("POTHER_ATTR_TEXT                ", 25, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ea, "ParentPotherAttrText");
    padview[44] = PadView.getInstance("PTYPE_TKEY_ATTR_TEXT            ", 4, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ea, "ParentPtypeTkeyAttrText");
    padview[45] = PadView.getInstance("                                ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_ea, 
      null);
    padview[46] = PadView.getInstance("TYPE                            ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, 
      w_ea, null);
    padview[47] = PadView.getInstance("TKEY_ATTR_TEXT                  ", 4, 0, 0, (short)3, PadView.VT_PREDICATE, PadView.DT_TEXT, 
      w_ea, "TypeTkeyAttrText");
    padview[48] = PadView.getInstance("LOCAL                           ", 0, 0, 0, (short)1, PadView.VT_BASE, PadView.DT_NONE, w_la, 
      "w_la");
    padview[49] = PadView.getInstance("LOC                             ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_la, 
      null);
    padview[50] = PadView.getInstance("DONT_CHANGE_RETURN_CODES        ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, 
      w_la, null);
    padview[51] = PadView.getInstance("1_OK                            ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReturnCodesQ1Ok");
    padview[52] = PadView.getInstance("N10_OBJ_NOT_FOUND               ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReturnCodesN10ObjNotFound");
    padview[53] = PadView.getInstance("N41_OBJ_UPDATE_FAILED           ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReturnCodesN41ObjUpdateFailed");
    padview[54] = PadView.getInstance("LOC                             ", 0, 0, 0, (short)2, PadView.VT_VIEW, PadView.DT_NONE, w_la, 
      null);
    padview[55] = PadView.getInstance("DONT_CHANGE_REASON_CODES        ", 0, 0, 0, (short)3, PadView.VT_ENTITY, PadView.DT_NONE, 
      w_la, null);
    padview[56] = PadView.getInstance("1_DEFAULT                       ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReasonCodesQ1Default");
    padview[57] = PadView.getInstance("101_PARENT_NOT_FOUND            ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReasonCodesQ101ParentNotFound");
    padview[58] = PadView.getInstance("102_PARENT_ALREADY_EXIST        ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReasonCodesQ102ParentAlreadyExist");
    padview[59] = PadView.getInstance("103_PARENT_ATTR_VALUE_INVALID   ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReasonCodesQ103ParentAttrValueInvalid");
    padview[60] = PadView.getInstance("111_PARENTS_TYPE_NOT_FOUND      ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReasonCodesQ111ParentsTypeNotFound");
    padview[61] = PadView.getInstance("112_PARENT_CONCURRENCY_ERROR    ", 5, 0, 0, (short)3, PadView.VT_PREDICATE, 
      PadView.DT_NUMERIC, w_la, "LocDontChangeReasonCodesQ112ParentConcurrencyError");
    padview[62] = PadView.getInstance("******** BOTTOM OF DATA ********", 0, 0, 0, (short)0, PadView.VT_END, PadView.DT_NONE, w_la, 
      null);
  }
  public void f_29360149()
  	throws ABException
  {
    sl_29360149 = ErrorData.SUCCEEDS_FL_LS;
    Parent_001cd = ABBase.PV_SELECT;
    Parent_lk = ABBase.EVW_UNLOCKED;
    f_29360149_moveb();
    
    if ( sl_29360149.equals( ErrorData.SUCCEEDS_FL_LS ) == true )
    {
      valChkDeadlockTimeout = false;
      valChkDBError = false;
      Parent_es = ABBase.EV_USABLE;
      Parent_lk = ABBase.EVW_LOCKED;
      if ( curs_open_0029360149 == true )
      {
        curs_open_0029360149 = false;
        try {
          if ( hstmt_0037926947_1_rs != null )
          {
            hstmt_0037926947_1_rs.close();
            hstmt_0037926947_1_rs = null;
          }
          if ( hstmt_0037926947_1 != null )
          {
            hstmt_0037926947_1.close();
            hstmt_0037926947_1 = null;
          }
        } catch( SQLException e ) {
          if ( globdata.getDBMSData().getDBMSManager().wasSuccessful(e) == false )
          {
            sqlException = e;
            f_29360149_jdbc_dberror();
          }
        }
      }
      
      sqlException = null;
      try {
        SQLStatement.setLength(0);
        SQLStatement.append("SELECT ");
        SQLStatement.append("VDVYYYP01.`PINSTANCE_ID`,");
        SQLStatement.append("VDVYYYP01.`PREFERENCE_ID`,");
        SQLStatement.append("VDVYYYP01.`PUPDATE_USERID`,");
        SQLStatement.append("VDVYYYP01.`PKEY_ATTR`,");
        SQLStatement.append("VDVYYYP01.`PSEARCH_ATTR`,");
        SQLStatement.append("VDVYYYP01.`POTHER_ATTR`,");
        SQLStatement.append("VDVYYYP01.`PTYPE_TKEY_ATTR_TE`");
        SQLStatement.append(" FROM ");
        SQLStatement.append("`VDVYYYP` VDVYYYP01");
        SQLStatement.append(" WHERE ");
        SQLStatement.append("(");
        SQLStatement.append("VDVYYYP01.`PKEY_ATTR` = ?");
        SQLStatement.append(")");
        SQLStatement.append(" FOR UPDATE OF `PINSTANCE_ID` ");
        hstmt_0037926947_1 = globdata.getDBMSData().getDBMSManager().getStatement(globdata, iefRuntimeParm2, "IEFDB", 
          SQLStatement.toString());
        globdata.getDBMSData().getDBMSManager().bindParameterString("IEFDB", hstmt_0037926947_1, 1, PkeyAttrText_001TP);
        hstmt_0037926947_1.execute();
        hstmt_0037926947_1_rs = hstmt_0037926947_1.getResultSet();
      } catch( SQLException e ) {
        sqlException = e;
      }
      if ( (globdata.getDBMSData().getDBMSManager().wasSuccessful(sqlException)) )
      {
        curs_open_0029360149 = true;
        sqlException = null;
        try {
          if ( hstmt_0037926947_1_rs.next() == false )
          {
            throw new SQLException("No data found");
          }
          pinstance_id_009EF = hstmt_0037926947_1_rs.getTimestamp(1);
          preference_id_011EF = hstmt_0037926947_1_rs.getTimestamp(2);
          pupdate_userid_007EF = globdata.getDBMSData().getDBMSManager().getStringFromResultSet("IEFDB", hstmt_0037926947_1_rs, 3);
          pkey_attr_001EF = globdata.getDBMSData().getDBMSManager().getStringFromResultSet("IEFDB", hstmt_0037926947_1_rs, 4);
          psearch_attr_003EF = globdata.getDBMSData().getDBMSManager().getStringFromResultSet("IEFDB", hstmt_0037926947_1_rs, 5);
          pother_attr_005EF = globdata.getDBMSData().getDBMSManager().getStringFromResultSet("IEFDB", hstmt_0037926947_1_rs, 6);
          ptype_tkey_attr_te_013EF = globdata.getDBMSData().getDBMSManager().getStringFromResultSet("IEFDB", hstmt_0037926947_1_rs, 
            7);
        } catch( SQLException e ) {
          sqlException = e;
        }
        
      }
      
      if ( globdata.getDBMSData().getDBMSManager().wasSuccessful(sqlException) )
      {
        f_29360149_movea();
        if ( sl_29360149.equals( ErrorData.SUCCEEDS_FL_LS ) )
        {
          Parent_es = ABBase.EV_USABLE;
          Parent_lk = ABBase.EVW_LOCKED;
        }
        else {
          Parent_es = ABBase.EV_UNUSABLE;
          Parent_lk = ABBase.EVW_UNLOCKED;
        }
      }
      else if ( globdata.getDBMSData().getDBMSManager().wasNoDataFound(sqlException) )
      {
        sl_29360149 = ErrorData.NOT_FOUND_FL_LS;
        Parent_es = ABBase.EV_UNUSABLE;
        Parent_lk = ABBase.EVW_UNLOCKED;
      }
      else {
        f_29360149_jdbc_dberror();
      }
    }
  }
  
  public void f_29360149_jdbc_dberror()
  	throws ABException
  {
    globdata.getErrorData().setStatus( ErrorData.FATAL_ERROR_FL_LS );
    globdata.getDBMSData().setActionId( 20 );
    globdata.getErrorData().setLastStatus( ErrorData.DB_ERROR_FL_LS );
    if ( sqlException != null )
    {
      globdata.getErrorData().setErrorMessage(sqlException);
      if ( globdata.getDBMSData().getDBMSManager().wasDeadlockOrTimeout("IEFDB", sqlException) )
      {
        globdata.getErrorData().setLastStatus( ErrorData.DB_DEADLOCK_TIMEOUT_FL_LS );
      }
    }
    sl_29360149 = globdata.getErrorData().getLastStatus();
    throw new ABException();
  }
  
  public void f_29360149_moveb()
  	throws ABException
  {
    PkeyAttrText_001TP = w_ia.ImpParentPkeyAttrText;
  }
  
  public void f_29360149_movea()
  	throws ABException
  {
    w_ea.ParentPinstanceId = TimestampAttr.valueOf(TIRS2VW.TIRS2VW(globdata, "IEFDB", pinstance_id_009EF));
    w_ea.ParentPreferenceId = TimestampAttr.valueOf(TIRS2VW.TIRS2VW(globdata, "IEFDB", preference_id_011EF));
    w_ea.ParentPupdateUserId = FixedStringAttr.valueOf(pupdate_userid_007EF, (short)8);
    w_ea.ParentPkeyAttrText = FixedStringAttr.valueOf(pkey_attr_001EF, (short)5);
    w_ea.ParentPsearchAttrText = FixedStringAttr.valueOf(psearch_attr_003EF, (short)25);
    w_ea.ParentPotherAttrText = FixedStringAttr.valueOf(pother_attr_005EF, (short)25);
    w_ea.ParentPtypeTkeyAttrText = FixedStringAttr.valueOf(ptype_tkey_attr_te_013EF, (short)4);
  }
  public void f_29360136()
  	throws ABException
  {
    sl_29360136 = ErrorData.SUCCEEDS_FL_LS;
    Type_003cd = ABBase.PV_SELECT;
    Type_lk = ABBase.EVW_UNLOCKED;
    f_29360136_moveb();
    
    if ( sl_29360136.equals( ErrorData.SUCCEEDS_FL_LS ) == true )
    {
      valChkDeadlockTimeout = false;
      valChkDBError = false;
      Type_es = ABBase.EV_USABLE;
      sqlException = null;
      try {
        SQLStatement.setLength(0);
        SQLStatement.append("SELECT ");
        SQLStatement.append("VDVYYYT01.`TKEY_ATTR`");
        SQLStatement.append(" FROM ");
        SQLStatement.append("`VDVYYYT` VDVYYYT01");
        SQLStatement.append(" WHERE ");
        SQLStatement.append("(");
        SQLStatement.append("VDVYYYT01.`TKEY_ATTR` = ?");
        SQLStatement.append(")");
        hstmt_0037954566_1 = globdata.getDBMSData().getDBMSManager().getStatement(globdata, iefRuntimeParm2, "IEFDB", 
          SQLStatement.toString());
        globdata.getDBMSData().getDBMSManager().bindParameterString("IEFDB", hstmt_0037954566_1, 1, PtypeTkeyAttrText_002TP);
        hstmt_0037954566_1.execute();
        hstmt_0037954566_1_rs = hstmt_0037954566_1.getResultSet();
        if ( hstmt_0037954566_1_rs.next() == false )
        {
          throw new SQLException("No data found");
        }
        tkey_attr_015EF = globdata.getDBMSData().getDBMSManager().getStringFromResultSet("IEFDB", hstmt_0037954566_1_rs, 1);
        if ( hstmt_0037954566_1_rs.next() == true )
        {
          throw new SQLException("READ statement with SELECT ONLY property returned more than 1 row");
        }
      } catch( SQLException e ) {
        sqlException = e;
      }
      if ( globdata.getDBMSData().getDBMSManager().wasSuccessful(sqlException) )
      {
        f_29360136_movea();
        if ( sl_29360136.equals( ErrorData.SUCCEEDS_FL_LS ) )
        {
          Type_es = ABBase.EV_USABLE;
        }
        else {
          Type_es = ABBase.EV_UNUSABLE;
        }
        try {
          if ( hstmt_0037954566_1_rs != null )
          {
            hstmt_0037954566_1_rs.close();
            hstmt_0037954566_1_rs = null;
          }
          if ( hstmt_0037954566_1 != null )
          {
            hstmt_0037954566_1.close();
            hstmt_0037954566_1 = null;
          }
        } catch( SQLException e ) {
          if ( globdata.getDBMSData().getDBMSManager().wasSuccessful(e) == false )
          {
            sqlException = e;
            f_29360136_jdbc_dberror();
          }
        }
      }
      else if ( globdata.getDBMSData().getDBMSManager().wasNoDataFound(sqlException) )
      {
        sl_29360136 = ErrorData.NOT_FOUND_FL_LS;
        Type_es = ABBase.EV_UNUSABLE;
        try {
          if ( hstmt_0037954566_1_rs != null )
          {
            hstmt_0037954566_1_rs.close();
            hstmt_0037954566_1_rs = null;
          }
          if ( hstmt_0037954566_1 != null )
          {
            hstmt_0037954566_1.close();
            hstmt_0037954566_1 = null;
          }
        } catch( SQLException e ) {
          if ( globdata.getDBMSData().getDBMSManager().wasSuccessful(e) == false )
          {
            sqlException = e;
            f_29360136_jdbc_dberror();
          }
        }
      }
      else {
        f_29360136_jdbc_dberror();
      }
    }
  }
  
  public void f_29360136_jdbc_dberror()
  	throws ABException
  {
    globdata.getErrorData().setStatus( ErrorData.FATAL_ERROR_FL_LS );
    globdata.getDBMSData().setActionId( 27 );
    globdata.getErrorData().setLastStatus( ErrorData.DB_ERROR_FL_LS );
    if ( sqlException != null )
    {
      globdata.getErrorData().setErrorMessage(sqlException);
      if ( globdata.getDBMSData().getDBMSManager().wasDeadlockOrTimeout("IEFDB", sqlException) )
      {
        globdata.getErrorData().setLastStatus( ErrorData.DB_DEADLOCK_TIMEOUT_FL_LS );
      }
    }
    sl_29360136 = globdata.getErrorData().getLastStatus();
    throw new ABException();
  }
  
  public void f_29360136_moveb()
  	throws ABException
  {
    PtypeTkeyAttrText_002TP = w_ia.ImpParentPtypeTkeyAttrText;
  }
  
  public void f_29360136_movea()
  	throws ABException
  {
    w_ea.TypeTkeyAttrText = FixedStringAttr.valueOf(tkey_attr_015EF, (short)4);
  }
  public void f_27263003_moveb()
  {
    if ( (Parent_es.equals( ABBase.EV_UNUSABLE ) == true) || (Parent_lk.equals( ABBase.EVW_UNLOCKED ) == true) )
    {
      up_27263003 = ABBase.EV_UNUSABLE;
      return;
    }
    preference_id_012EN = TIRVW2S.TIRVW2S(globdata, "IEFDB", w_ea.ParentPreferenceId);
    pupdate_userid_008EN = w_ea.ParentPupdateUserId;
    psearch_attr_004EN = w_ea.ParentPsearchAttrText;
    pother_attr_006EN = w_ea.ParentPotherAttrText;
    ptype_tkey_attr_te_014EN = w_ea.ParentPtypeTkeyAttrText;
  }
  public void f_27263003()
  	throws ABException
  {
    up_27263003 = ErrorData.SUCCEEDS_FL_LS;
    Parent_001cd = ABBase.PV_UPDATE;
    f_27263003_moveb();
    
    if ( up_27263003.equals( ErrorData.SUCCEEDS_FL_LS ) == true )
    {
      sqlException = null;
      try {
        SQLStatement.setLength(0);
        SQLStatement.append("UPDATE ");
        SQLStatement.append("`VDVYYYP`");
        SQLStatement.append(" SET ");
        SQLStatement.append("`PREFERENCE_ID` = ?,");
        SQLStatement.append("`PUPDATE_USERID` = ?,");
        SQLStatement.append("`PSEARCH_ATTR` = ?,");
        SQLStatement.append("`POTHER_ATTR` = ?,");
        SQLStatement.append("`PTYPE_TKEY_ATTR_TE` = ?");
        SQLStatement.append(" WHERE ");
        SQLStatement.append("`PKEY_ATTR` = ?");
        hstmt_0027263003_1 = globdata.getDBMSData().getDBMSManager().getStatement(globdata, iefRuntimeParm2, "IEFDB", 
          SQLStatement.toString());
        hstmt_0027263003_1.setTimestamp(1, preference_id_012EN);
        globdata.getDBMSData().getDBMSManager().bindParameterString("IEFDB", hstmt_0027263003_1, 2, pupdate_userid_008EN);
        globdata.getDBMSData().getDBMSManager().bindParameterString("IEFDB", hstmt_0027263003_1, 3, psearch_attr_004EN);
        globdata.getDBMSData().getDBMSManager().bindParameterString("IEFDB", hstmt_0027263003_1, 4, pother_attr_006EN);
        globdata.getDBMSData().getDBMSManager().bindParameterString("IEFDB", hstmt_0027263003_1, 5, ptype_tkey_attr_te_014EN);
        globdata.getDBMSData().getDBMSManager().bindParameterString("IEFDB", hstmt_0027263003_1, 6, pkey_attr_001EF);
        hstmt_0027263003_1.execute();
        hstmt_0027263003_1_rs = hstmt_0027263003_1.getResultSet();
        hstmt_0027263003_1_cnt = hstmt_0027263003_1.getUpdateCount();
      } catch( SQLException e ) {
        sqlException = e;
      }
      if ( (globdata.getDBMSData().getDBMSManager().wasSuccessful(sqlException) && hstmt_0027263003_1_cnt == 1) )
      {
        preference_id_011EF = preference_id_012EN;
        pupdate_userid_007EF = pupdate_userid_008EN;
        psearch_attr_003EF = psearch_attr_004EN;
        pother_attr_005EF = pother_attr_006EN;
        ptype_tkey_attr_te_013EF = ptype_tkey_attr_te_014EN;
        Parent_es = ABBase.EV_USABLE;
        Parent_lk = ABBase.EVW_LOCKED;
        try {
          if ( hstmt_0027263003_1_rs != null )
          {
            hstmt_0027263003_1_rs.close();
            hstmt_0027263003_1_rs = null;
          }
          if ( hstmt_0027263003_1 != null )
          {
            hstmt_0027263003_1.close();
            hstmt_0027263003_1 = null;
          }
        } catch( SQLException e ) {
          if ( globdata.getDBMSData().getDBMSManager().wasSuccessful(e) == false )
          {
            sqlException = e;
            f_27263003_jdbc_dberror();
          }
        }
      }
      else if ( globdata.getDBMSData().getDBMSManager().wasDuplicateFound("IEFDB", sqlException) )
      {
        up_27263003 = ErrorData.DUPLICATE_FOUND_FL_LS;
        Parent_es = ABBase.EV_UNUSABLE;
        try {
          if ( hstmt_0027263003_1_rs != null )
          {
            hstmt_0027263003_1_rs.close();
            hstmt_0027263003_1_rs = null;
          }
          if ( hstmt_0027263003_1 != null )
          {
            hstmt_0027263003_1.close();
            hstmt_0027263003_1 = null;
          }
        } catch( SQLException e ) {
          if ( globdata.getDBMSData().getDBMSManager().wasSuccessful(e) == false )
          {
            sqlException = e;
            f_27263003_jdbc_dberror();
          }
        }
      }
      else {
        f_27263003_jdbc_dberror();
      }
    }
    else if ( up_27263003.equals( ErrorData.INVALID_DATAB_PERM_FL_LS ) == false )
    {
      globdata.getErrorData().setStatus( ErrorData.FATAL_ERROR_FL_LS );
      globdata.getDBMSData().setActionId( 29 );
      globdata.getErrorData().setLastStatus( up_27263003 );
      throw new ABException();
    }
    return;
  }
  
  public void f_27263003_jdbc_dberror()
  	throws ABException
  {
    globdata.getErrorData().setStatus( ErrorData.FATAL_ERROR_FL_LS );
    globdata.getDBMSData().setActionId( 29 );
    globdata.getErrorData().setLastStatus( ErrorData.DB_ERROR_FL_LS );
    if ( sqlException != null )
    {
      globdata.getErrorData().setErrorMessage(sqlException);
      if ( globdata.getDBMSData().getDBMSManager().wasDeadlockOrTimeout("IEFDB", sqlException) )
      {
        globdata.getErrorData().setLastStatus( ErrorData.DB_DEADLOCK_TIMEOUT_FL_LS );
      }
    }
    up_27263003 = globdata.getErrorData().getLastStatus();
    throw new ABException();
  }
  
  public void f_22020301_term()
  {
    
    // * * * * * * * * * *
    // DASG Term Function 
    // * * * * * * * * * *
    
    try {
      if ( curs_open_0029360149 == true )
      {
        curs_open_0029360149 = false;
        try {
          if ( hstmt_0037926947_1_rs != null )
          {
            hstmt_0037926947_1_rs.close();
            hstmt_0037926947_1_rs = null;
          }
          if ( hstmt_0037926947_1 != null )
          {
            hstmt_0037926947_1.close();
            hstmt_0037926947_1 = null;
          }
        } catch( SQLException e ) {
          if ( globdata.getDBMSData().getDBMSManager().wasSuccessful(e) == false )
          {
            sqlException = e;
            f_29360149_jdbc_dberror();
          }
        }
      }
    } catch( ABException e ) {
      // No action can be taken here.
    }
  }
}// end class


package gen.ort.yyy;
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
//
//                    Source Code Generated by
//                           CA Gen 8.6
//
//    Copyright (c) 2023 CA Technologies. All rights reserved.
//
//    Name: DYYY0131_IA                      Date: 2023/07/05
//    User: HakanBal                         Time: 15:08:21
//
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// Import Statements
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
import java.lang.*;
import java.io.*;
import java.util.*;
import java.math.*;
import java.sql.*;
import com.ca.gen.vwrt.*;
import com.ca.gen.vwrt.types.*;
import com.ca.gen.vwrt.vdf.*;
import com.ca.gen.csu.exception.*;

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// START OF IMPORT VIEW
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
/**
 * Internal data view storage for: DYYY0131_IA
 **/
public class DYYY0131_IA extends ViewBase implements IImportView, Serializable
{
  // Entity View: IMP_ERROR
  //        Type: IYY1_COMPONENT
  /**
   * Attribute missing flag for: ImpErrorIyy1ComponentSeverityCode
   **/
  public char ImpErrorIyy1ComponentSeverityCode_AS;
  /**
   * Attribute for: ImpErrorIyy1ComponentSeverityCode
   * Domain: Text
   * Length: 1
   * Varying Length: N
   **/
  public String ImpErrorIyy1ComponentSeverityCode;
  /**
   * Attribute missing flag for: ImpErrorIyy1ComponentRollbackIndicator
   **/
  public char ImpErrorIyy1ComponentRollbackIndicator_AS;
  /**
   * Attribute for: ImpErrorIyy1ComponentRollbackIndicator
   * Domain: Text
   * Length: 1
   * Varying Length: N
   **/
  public String ImpErrorIyy1ComponentRollbackIndicator;
  /**
   * Attribute missing flag for: ImpErrorIyy1ComponentOriginServid
   **/
  public char ImpErrorIyy1ComponentOriginServid_AS;
  /**
   * Attribute for: ImpErrorIyy1ComponentOriginServid
   * Domain: Number
   * Length: 15
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public double ImpErrorIyy1ComponentOriginServid;
  /**
   * Attribute missing flag for: ImpErrorIyy1ComponentContextString
   **/
  public char ImpErrorIyy1ComponentContextString_AS;
  /**
   * Attribute for: ImpErrorIyy1ComponentContextString
   * Domain: Text
   * Length: 512
   * Varying Length: Y
   **/
  public String ImpErrorIyy1ComponentContextString;
  /**
   * Attribute missing flag for: ImpErrorIyy1ComponentReturnCode
   **/
  public char ImpErrorIyy1ComponentReturnCode_AS;
  /**
   * Attribute for: ImpErrorIyy1ComponentReturnCode
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int ImpErrorIyy1ComponentReturnCode;
  /**
   * Attribute missing flag for: ImpErrorIyy1ComponentReasonCode
   **/
  public char ImpErrorIyy1ComponentReasonCode_AS;
  /**
   * Attribute for: ImpErrorIyy1ComponentReasonCode
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int ImpErrorIyy1ComponentReasonCode;
  /**
   * Attribute missing flag for: ImpErrorIyy1ComponentChecksum
   **/
  public char ImpErrorIyy1ComponentChecksum_AS;
  /**
   * Attribute for: ImpErrorIyy1ComponentChecksum
   * Domain: Text
   * Length: 15
   * Varying Length: N
   **/
  public String ImpErrorIyy1ComponentChecksum;
  // Entity View: IMP_REFERENCE
  //        Type: IYY1_SERVER_DATA
  /**
   * Attribute missing flag for: ImpReferenceIyy1ServerDataUserid
   **/
  public char ImpReferenceIyy1ServerDataUserid_AS;
  /**
   * Attribute for: ImpReferenceIyy1ServerDataUserid
   * Domain: Text
   * Length: 8
   * Varying Length: N
   **/
  public String ImpReferenceIyy1ServerDataUserid;
  /**
   * Attribute missing flag for: ImpReferenceIyy1ServerDataReferenceId
   **/
  public char ImpReferenceIyy1ServerDataReferenceId_AS;
  /**
   * Attribute for: ImpReferenceIyy1ServerDataReferenceId
   * Domain: Timestamp
   * Length: 20
   **/
  public String ImpReferenceIyy1ServerDataReferenceId;
  // Entity View: IMP
  //        Type: PARENT
  /**
   * Attribute missing flag for: ImpParentPinstanceId
   **/
  public char ImpParentPinstanceId_AS;
  /**
   * Attribute for: ImpParentPinstanceId
   * Domain: Timestamp
   * Length: 20
   **/
  public String ImpParentPinstanceId;
  /**
   * Attribute missing flag for: ImpParentPreferenceId
   **/
  public char ImpParentPreferenceId_AS;
  /**
   * Attribute for: ImpParentPreferenceId
   * Domain: Timestamp
   * Length: 20
   **/
  public String ImpParentPreferenceId;
  /**
   * Attribute missing flag for: ImpParentPkeyAttrText
   **/
  public char ImpParentPkeyAttrText_AS;
  /**
   * Attribute for: ImpParentPkeyAttrText
   * Domain: Text
   * Length: 5
   * Varying Length: N
   **/
  public String ImpParentPkeyAttrText;
  /**
   * Attribute missing flag for: ImpParentPsearchAttrText
   **/
  public char ImpParentPsearchAttrText_AS;
  /**
   * Attribute for: ImpParentPsearchAttrText
   * Domain: Text
   * Length: 25
   * Varying Length: N
   **/
  public String ImpParentPsearchAttrText;
  /**
   * Attribute missing flag for: ImpParentPotherAttrText
   **/
  public char ImpParentPotherAttrText_AS;
  /**
   * Attribute for: ImpParentPotherAttrText
   * Domain: Text
   * Length: 25
   * Varying Length: N
   **/
  public String ImpParentPotherAttrText;
  /**
   * Attribute missing flag for: ImpParentPtypeTkeyAttrText
   **/
  public char ImpParentPtypeTkeyAttrText_AS;
  /**
   * Attribute for: ImpParentPtypeTkeyAttrText
   * Domain: Text
   * Length: 4
   * Varying Length: N
   **/
  public String ImpParentPtypeTkeyAttrText;
  /**
   * Default Constructor
   **/
  
  public DYYY0131_IA()
  {
    reset();
  }
  /**
   * Copy Constructor
   **/
  
  public DYYY0131_IA(DYYY0131_IA orig)
  {
    copyFrom(orig);
  }
  /**
   * Static instance creator function
   **/
  
  public static DYYY0131_IA getInstance()
  {
    return new DYYY0131_IA();
  }
  /**
   * Static free instance method
   **/
  
  public void freeInstance()
  {
  }
  /**
   * clone constructor
   **/
  
  @Override public Object clone()
  	throws CloneNotSupportedException
  {
    return(new DYYY0131_IA(this));
  }
  /**
   * Resets all properties to the defaults.
   **/
  
  public void reset()
  {
    ImpErrorIyy1ComponentSeverityCode_AS = ' ';
    ImpErrorIyy1ComponentSeverityCode = " ";
    ImpErrorIyy1ComponentRollbackIndicator_AS = ' ';
    ImpErrorIyy1ComponentRollbackIndicator = " ";
    ImpErrorIyy1ComponentOriginServid_AS = ' ';
    ImpErrorIyy1ComponentOriginServid = 0.0;
    ImpErrorIyy1ComponentContextString_AS = ' ';
    ImpErrorIyy1ComponentContextString = "";
    ImpErrorIyy1ComponentReturnCode_AS = ' ';
    ImpErrorIyy1ComponentReturnCode = 0;
    ImpErrorIyy1ComponentReasonCode_AS = ' ';
    ImpErrorIyy1ComponentReasonCode = 0;
    ImpErrorIyy1ComponentChecksum_AS = ' ';
    ImpErrorIyy1ComponentChecksum = "               ";
    ImpReferenceIyy1ServerDataUserid_AS = ' ';
    ImpReferenceIyy1ServerDataUserid = "        ";
    ImpReferenceIyy1ServerDataReferenceId_AS = ' ';
    ImpReferenceIyy1ServerDataReferenceId = "00000000000000000000";
    ImpParentPinstanceId_AS = ' ';
    ImpParentPinstanceId = "00000000000000000000";
    ImpParentPreferenceId_AS = ' ';
    ImpParentPreferenceId = "00000000000000000000";
    ImpParentPkeyAttrText_AS = ' ';
    ImpParentPkeyAttrText = "     ";
    ImpParentPsearchAttrText_AS = ' ';
    ImpParentPsearchAttrText = "                         ";
    ImpParentPotherAttrText_AS = ' ';
    ImpParentPotherAttrText = "                         ";
    ImpParentPtypeTkeyAttrText_AS = ' ';
    ImpParentPtypeTkeyAttrText = "    ";
  }
  /**
   * Gets the VDF version of the current state of the instance.
   **/
  public VDF getVDF()
  {
    throw new RuntimeException("can only execute getVDF for a Procedure Step.");
  }
  
  /**
   * Sets the current state of the instance to the VDF version.
   **/
  public void setFromVDF(VDF vdf)
  {
    throw new RuntimeException("can only execute setFromVDF for a Procedure Step.");
  }
  
  /**
   * Sets the current instance based on the passed view.
   **/
  public void copyFrom(IImportView orig)
  {
    this.copyFrom((DYYY0131_IA) orig);
  }
  
  /**
   * Sets the current instance based on the passed view.
   **/
  public void copyFrom(DYYY0131_IA orig)
  {
    ImpErrorIyy1ComponentSeverityCode_AS = orig.ImpErrorIyy1ComponentSeverityCode_AS;
    ImpErrorIyy1ComponentSeverityCode = orig.ImpErrorIyy1ComponentSeverityCode;
    ImpErrorIyy1ComponentRollbackIndicator_AS = orig.ImpErrorIyy1ComponentRollbackIndicator_AS;
    ImpErrorIyy1ComponentRollbackIndicator = orig.ImpErrorIyy1ComponentRollbackIndicator;
    ImpErrorIyy1ComponentOriginServid_AS = orig.ImpErrorIyy1ComponentOriginServid_AS;
    ImpErrorIyy1ComponentOriginServid = orig.ImpErrorIyy1ComponentOriginServid;
    ImpErrorIyy1ComponentContextString_AS = orig.ImpErrorIyy1ComponentContextString_AS;
    ImpErrorIyy1ComponentContextString = orig.ImpErrorIyy1ComponentContextString;
    ImpErrorIyy1ComponentReturnCode_AS = orig.ImpErrorIyy1ComponentReturnCode_AS;
    ImpErrorIyy1ComponentReturnCode = orig.ImpErrorIyy1ComponentReturnCode;
    ImpErrorIyy1ComponentReasonCode_AS = orig.ImpErrorIyy1ComponentReasonCode_AS;
    ImpErrorIyy1ComponentReasonCode = orig.ImpErrorIyy1ComponentReasonCode;
    ImpErrorIyy1ComponentChecksum_AS = orig.ImpErrorIyy1ComponentChecksum_AS;
    ImpErrorIyy1ComponentChecksum = orig.ImpErrorIyy1ComponentChecksum;
    ImpReferenceIyy1ServerDataUserid_AS = orig.ImpReferenceIyy1ServerDataUserid_AS;
    ImpReferenceIyy1ServerDataUserid = orig.ImpReferenceIyy1ServerDataUserid;
    ImpReferenceIyy1ServerDataReferenceId_AS = orig.ImpReferenceIyy1ServerDataReferenceId_AS;
    ImpReferenceIyy1ServerDataReferenceId = orig.ImpReferenceIyy1ServerDataReferenceId;
    ImpParentPinstanceId_AS = orig.ImpParentPinstanceId_AS;
    ImpParentPinstanceId = orig.ImpParentPinstanceId;
    ImpParentPreferenceId_AS = orig.ImpParentPreferenceId_AS;
    ImpParentPreferenceId = orig.ImpParentPreferenceId;
    ImpParentPkeyAttrText_AS = orig.ImpParentPkeyAttrText_AS;
    ImpParentPkeyAttrText = orig.ImpParentPkeyAttrText;
    ImpParentPsearchAttrText_AS = orig.ImpParentPsearchAttrText_AS;
    ImpParentPsearchAttrText = orig.ImpParentPsearchAttrText;
    ImpParentPotherAttrText_AS = orig.ImpParentPotherAttrText_AS;
    ImpParentPotherAttrText = orig.ImpParentPotherAttrText;
    ImpParentPtypeTkeyAttrText_AS = orig.ImpParentPtypeTkeyAttrText_AS;
    ImpParentPtypeTkeyAttrText = orig.ImpParentPtypeTkeyAttrText;
  }
}

package gen.ort.yyy;
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
//
//                    Source Code Generated by
//                           CA Gen 8.6
//
//    Copyright (c) 2023 CA Technologies. All rights reserved.
//
//    Name: DYYY0131_LA                      Date: 2023/07/05
//    User: HakanBal                         Time: 15:08:21
//
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// Import Statements
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
import java.lang.*;
import java.io.*;
import java.util.*;
import java.math.*;
import java.sql.*;
import com.ca.gen.vwrt.*;
import com.ca.gen.vwrt.types.*;
import com.ca.gen.vwrt.vdf.*;
import com.ca.gen.csu.exception.*;

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// START OF LOCAL VIEW
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
/**
 * Internal data view storage for: DYYY0131_LA
 **/
public class DYYY0131_LA extends ViewBase implements ILocalView, Serializable
{
  // Entity View: LOC
  //        Type: DONT_CHANGE_RETURN_CODES
  /**
   * Attribute missing flag for: LocDontChangeReturnCodesQ1Ok
   **/
  public char LocDontChangeReturnCodesQ1Ok_AS;
  /**
   * Attribute for: LocDontChangeReturnCodesQ1Ok
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReturnCodesQ1Ok;
  /**
   * Attribute missing flag for: LocDontChangeReturnCodesN10ObjNotFound
   **/
  public char LocDontChangeReturnCodesN10ObjNotFound_AS;
  /**
   * Attribute for: LocDontChangeReturnCodesN10ObjNotFound
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReturnCodesN10ObjNotFound;
  /**
   * Attribute missing flag for: LocDontChangeReturnCodesN41ObjUpdateFailed
   **/
  public char LocDontChangeReturnCodesN41ObjUpdateFailed_AS;
  /**
   * Attribute for: LocDontChangeReturnCodesN41ObjUpdateFailed
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReturnCodesN41ObjUpdateFailed;
  // Entity View: LOC
  //        Type: DONT_CHANGE_REASON_CODES
  /**
   * Attribute missing flag for: LocDontChangeReasonCodesQ1Default
   **/
  public char LocDontChangeReasonCodesQ1Default_AS;
  /**
   * Attribute for: LocDontChangeReasonCodesQ1Default
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReasonCodesQ1Default;
  /**
   * Attribute missing flag for: LocDontChangeReasonCodesQ101ParentNotFound
   **/
  public char LocDontChangeReasonCodesQ101ParentNotFound_AS;
  /**
   * Attribute for: LocDontChangeReasonCodesQ101ParentNotFound
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReasonCodesQ101ParentNotFound;
  /**
   * Attribute missing flag for: LocDontChangeReasonCodesQ102ParentAlreadyExist
   **/
  public char LocDontChangeReasonCodesQ102ParentAlreadyExist_AS;
  /**
   * Attribute for: LocDontChangeReasonCodesQ102ParentAlreadyExist
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReasonCodesQ102ParentAlreadyExist;
  /**
   * Attribute missing flag for: LocDontChangeReasonCodesQ103ParentAttrValueInvalid
   **/
  public char LocDontChangeReasonCodesQ103ParentAttrValueInvalid_AS;
  /**
   * Attribute for: LocDontChangeReasonCodesQ103ParentAttrValueInvalid
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReasonCodesQ103ParentAttrValueInvalid;
  /**
   * Attribute missing flag for: LocDontChangeReasonCodesQ111ParentsTypeNotFound
   **/
  public char LocDontChangeReasonCodesQ111ParentsTypeNotFound_AS;
  /**
   * Attribute for: LocDontChangeReasonCodesQ111ParentsTypeNotFound
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReasonCodesQ111ParentsTypeNotFound;
  /**
   * Attribute missing flag for: LocDontChangeReasonCodesQ112ParentConcurrencyError
   **/
  public char LocDontChangeReasonCodesQ112ParentConcurrencyError_AS;
  /**
   * Attribute for: LocDontChangeReasonCodesQ112ParentConcurrencyError
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int LocDontChangeReasonCodesQ112ParentConcurrencyError;
  /**
   * Default Constructor
   **/
  
  public DYYY0131_LA()
  {
    reset();
  }
  /**
   * Copy Constructor
   **/
  
  public DYYY0131_LA(DYYY0131_LA orig)
  {
    LocDontChangeReturnCodesQ1Ok_AS = orig.LocDontChangeReturnCodesQ1Ok_AS;
    LocDontChangeReturnCodesQ1Ok = orig.LocDontChangeReturnCodesQ1Ok;
    LocDontChangeReturnCodesN10ObjNotFound_AS = orig.LocDontChangeReturnCodesN10ObjNotFound_AS;
    LocDontChangeReturnCodesN10ObjNotFound = orig.LocDontChangeReturnCodesN10ObjNotFound;
    LocDontChangeReturnCodesN41ObjUpdateFailed_AS = orig.LocDontChangeReturnCodesN41ObjUpdateFailed_AS;
    LocDontChangeReturnCodesN41ObjUpdateFailed = orig.LocDontChangeReturnCodesN41ObjUpdateFailed;
    LocDontChangeReasonCodesQ1Default_AS = orig.LocDontChangeReasonCodesQ1Default_AS;
    LocDontChangeReasonCodesQ1Default = orig.LocDontChangeReasonCodesQ1Default;
    LocDontChangeReasonCodesQ101ParentNotFound_AS = orig.LocDontChangeReasonCodesQ101ParentNotFound_AS;
    LocDontChangeReasonCodesQ101ParentNotFound = orig.LocDontChangeReasonCodesQ101ParentNotFound;
    LocDontChangeReasonCodesQ102ParentAlreadyExist_AS = orig.LocDontChangeReasonCodesQ102ParentAlreadyExist_AS;
    LocDontChangeReasonCodesQ102ParentAlreadyExist = orig.LocDontChangeReasonCodesQ102ParentAlreadyExist;
    LocDontChangeReasonCodesQ103ParentAttrValueInvalid_AS = orig.LocDontChangeReasonCodesQ103ParentAttrValueInvalid_AS;
    LocDontChangeReasonCodesQ103ParentAttrValueInvalid = orig.LocDontChangeReasonCodesQ103ParentAttrValueInvalid;
    LocDontChangeReasonCodesQ111ParentsTypeNotFound_AS = orig.LocDontChangeReasonCodesQ111ParentsTypeNotFound_AS;
    LocDontChangeReasonCodesQ111ParentsTypeNotFound = orig.LocDontChangeReasonCodesQ111ParentsTypeNotFound;
    LocDontChangeReasonCodesQ112ParentConcurrencyError_AS = orig.LocDontChangeReasonCodesQ112ParentConcurrencyError_AS;
    LocDontChangeReasonCodesQ112ParentConcurrencyError = orig.LocDontChangeReasonCodesQ112ParentConcurrencyError;
  }
  /**
   * Static instance creator function
   **/
  
  public static DYYY0131_LA getInstance()
  {
    return new DYYY0131_LA();
  }
  /**
   * Static free instance method
   **/
  
  public void freeInstance()
  {
  }
  /**
   * clone constructor
   **/
  
  @Override public Object clone()
  	throws CloneNotSupportedException
  {
    return(new DYYY0131_LA(this));
  }
  /**
   * Resets all properties to the defaults.
   **/
  
  public void reset()
  {
    LocDontChangeReturnCodesQ1Ok_AS = ' ';
    LocDontChangeReturnCodesQ1Ok = 0;
    LocDontChangeReturnCodesN10ObjNotFound_AS = ' ';
    LocDontChangeReturnCodesN10ObjNotFound = 0;
    LocDontChangeReturnCodesN41ObjUpdateFailed_AS = ' ';
    LocDontChangeReturnCodesN41ObjUpdateFailed = 0;
    LocDontChangeReasonCodesQ1Default_AS = ' ';
    LocDontChangeReasonCodesQ1Default = 0;
    LocDontChangeReasonCodesQ101ParentNotFound_AS = ' ';
    LocDontChangeReasonCodesQ101ParentNotFound = 0;
    LocDontChangeReasonCodesQ102ParentAlreadyExist_AS = ' ';
    LocDontChangeReasonCodesQ102ParentAlreadyExist = 0;
    LocDontChangeReasonCodesQ103ParentAttrValueInvalid_AS = ' ';
    LocDontChangeReasonCodesQ103ParentAttrValueInvalid = 0;
    LocDontChangeReasonCodesQ111ParentsTypeNotFound_AS = ' ';
    LocDontChangeReasonCodesQ111ParentsTypeNotFound = 0;
    LocDontChangeReasonCodesQ112ParentConcurrencyError_AS = ' ';
    LocDontChangeReasonCodesQ112ParentConcurrencyError = 0;
  }
}

package gen.ort.yyy;
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
//
//                    Source Code Generated by
//                           CA Gen 8.6
//
//    Copyright (c) 2023 CA Technologies. All rights reserved.
//
//    Name: DYYY0131_OA                      Date: 2023/07/05
//    User: HakanBal                         Time: 15:08:21
//
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// Import Statements
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
import java.lang.*;
import java.io.*;
import java.util.*;
import java.math.*;
import java.sql.*;
import com.ca.gen.vwrt.*;
import com.ca.gen.vwrt.types.*;
import com.ca.gen.vwrt.vdf.*;
import com.ca.gen.csu.exception.*;

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// START OF EXPORT VIEW
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
/**
 * Internal data view storage for: DYYY0131_OA
 **/
public class DYYY0131_OA extends ViewBase implements IExportView, Serializable
{
  // Entity View: EXP
  //        Type: PARENT
  /**
   * Attribute missing flag for: ExpParentPreferenceId
   **/
  public char ExpParentPreferenceId_AS;
  /**
   * Attribute for: ExpParentPreferenceId
   * Domain: Timestamp
   * Length: 20
   **/
  public String ExpParentPreferenceId;
  // Entity View: EXP_ERROR
  //        Type: IYY1_COMPONENT
  /**
   * Attribute missing flag for: ExpErrorIyy1ComponentSeverityCode
   **/
  public char ExpErrorIyy1ComponentSeverityCode_AS;
  /**
   * Attribute for: ExpErrorIyy1ComponentSeverityCode
   * Domain: Text
   * Length: 1
   * Varying Length: N
   **/
  public String ExpErrorIyy1ComponentSeverityCode;
  /**
   * Attribute missing flag for: ExpErrorIyy1ComponentRollbackIndicator
   **/
  public char ExpErrorIyy1ComponentRollbackIndicator_AS;
  /**
   * Attribute for: ExpErrorIyy1ComponentRollbackIndicator
   * Domain: Text
   * Length: 1
   * Varying Length: N
   **/
  public String ExpErrorIyy1ComponentRollbackIndicator;
  /**
   * Attribute missing flag for: ExpErrorIyy1ComponentOriginServid
   **/
  public char ExpErrorIyy1ComponentOriginServid_AS;
  /**
   * Attribute for: ExpErrorIyy1ComponentOriginServid
   * Domain: Number
   * Length: 15
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public double ExpErrorIyy1ComponentOriginServid;
  /**
   * Attribute missing flag for: ExpErrorIyy1ComponentContextString
   **/
  public char ExpErrorIyy1ComponentContextString_AS;
  /**
   * Attribute for: ExpErrorIyy1ComponentContextString
   * Domain: Text
   * Length: 512
   * Varying Length: Y
   **/
  public String ExpErrorIyy1ComponentContextString;
  /**
   * Attribute missing flag for: ExpErrorIyy1ComponentReturnCode
   **/
  public char ExpErrorIyy1ComponentReturnCode_AS;
  /**
   * Attribute for: ExpErrorIyy1ComponentReturnCode
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int ExpErrorIyy1ComponentReturnCode;
  /**
   * Attribute missing flag for: ExpErrorIyy1ComponentReasonCode
   **/
  public char ExpErrorIyy1ComponentReasonCode_AS;
  /**
   * Attribute for: ExpErrorIyy1ComponentReasonCode
   * Domain: Number
   * Length: 5
   * Decimal Places: 0
   * Decimal Precision: N
   **/
  public int ExpErrorIyy1ComponentReasonCode;
  /**
   * Attribute missing flag for: ExpErrorIyy1ComponentChecksum
   **/
  public char ExpErrorIyy1ComponentChecksum_AS;
  /**
   * Attribute for: ExpErrorIyy1ComponentChecksum
   * Domain: Text
   * Length: 15
   * Varying Length: N
   **/
  public String ExpErrorIyy1ComponentChecksum;
  /**
   * Default Constructor
   **/
  
  public DYYY0131_OA()
  {
    reset();
  }
  /**
   * Copy Constructor
   **/
  
  public DYYY0131_OA(DYYY0131_OA orig)
  {
    copyFrom(orig);
  }
  /**
   * Static instance creator function
   **/
  
  public static DYYY0131_OA getInstance()
  {
    return new DYYY0131_OA();
  }
  /**
   * Static free instance method
   **/
  
  public void freeInstance()
  {
  }
  /**
   * clone constructor
   **/
  
  @Override public Object clone()
  	throws CloneNotSupportedException
  {
    return(new DYYY0131_OA(this));
  }
  /**
   * Resets all properties to the defaults.
   **/
  
  public void reset()
  {
    ExpParentPreferenceId_AS = ' ';
    ExpParentPreferenceId = "00000000000000000000";
    ExpErrorIyy1ComponentSeverityCode_AS = ' ';
    ExpErrorIyy1ComponentSeverityCode = " ";
    ExpErrorIyy1ComponentRollbackIndicator_AS = ' ';
    ExpErrorIyy1ComponentRollbackIndicator = " ";
    ExpErrorIyy1ComponentOriginServid_AS = ' ';
    ExpErrorIyy1ComponentOriginServid = 0.0;
    ExpErrorIyy1ComponentContextString_AS = ' ';
    ExpErrorIyy1ComponentContextString = "";
    ExpErrorIyy1ComponentReturnCode_AS = ' ';
    ExpErrorIyy1ComponentReturnCode = 0;
    ExpErrorIyy1ComponentReasonCode_AS = ' ';
    ExpErrorIyy1ComponentReasonCode = 0;
    ExpErrorIyy1ComponentChecksum_AS = ' ';
    ExpErrorIyy1ComponentChecksum = "               ";
  }
  /**
   * Sets the current state of the instance to the VDF version.
   **/
  public void setFromVDF(VDF vdf)
  {
    throw new RuntimeException("can only execute setFromVDF for a Procedure Step.");
  }
  
  /**
   * Gets the VDF version of the current state of the instance.
   **/
  public VDF getVDF()
  {
    throw new RuntimeException("can only execute getVDF for a Procedure Step.");
  }
  
  /**
   * Sets the current instance based on the passed view.
   **/
  public void copyFrom(IExportView orig)
  {
    this.copyFrom((DYYY0131_OA) orig);
  }
  
  /**
   * Sets the current instance based on the passed view.
   **/
  public void copyFrom(DYYY0131_OA orig)
  {
    ExpParentPreferenceId_AS = orig.ExpParentPreferenceId_AS;
    ExpParentPreferenceId = orig.ExpParentPreferenceId;
    ExpErrorIyy1ComponentSeverityCode_AS = orig.ExpErrorIyy1ComponentSeverityCode_AS;
    ExpErrorIyy1ComponentSeverityCode = orig.ExpErrorIyy1ComponentSeverityCode;
    ExpErrorIyy1ComponentRollbackIndicator_AS = orig.ExpErrorIyy1ComponentRollbackIndicator_AS;
    ExpErrorIyy1ComponentRollbackIndicator = orig.ExpErrorIyy1ComponentRollbackIndicator;
    ExpErrorIyy1ComponentOriginServid_AS = orig.ExpErrorIyy1ComponentOriginServid_AS;
    ExpErrorIyy1ComponentOriginServid = orig.ExpErrorIyy1ComponentOriginServid;
    ExpErrorIyy1ComponentContextString_AS = orig.ExpErrorIyy1ComponentContextString_AS;
    ExpErrorIyy1ComponentContextString = orig.ExpErrorIyy1ComponentContextString;
    ExpErrorIyy1ComponentReturnCode_AS = orig.ExpErrorIyy1ComponentReturnCode_AS;
    ExpErrorIyy1ComponentReturnCode = orig.ExpErrorIyy1ComponentReturnCode;
    ExpErrorIyy1ComponentReasonCode_AS = orig.ExpErrorIyy1ComponentReasonCode_AS;
    ExpErrorIyy1ComponentReasonCode = orig.ExpErrorIyy1ComponentReasonCode;
    ExpErrorIyy1ComponentChecksum_AS = orig.ExpErrorIyy1ComponentChecksum_AS;
    ExpErrorIyy1ComponentChecksum = orig.ExpErrorIyy1ComponentChecksum;
  }
}
